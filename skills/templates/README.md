...
`workspace-context.md` 是显式工作区上下文模板。将其复制为 `docs/workspaces/workspace-<NNN>-<slug>/workspace.md`，绑定一个 Root Goal、该工作区根 canonical 范围、**必填**愿景 `plan_refs`/`primary_plan` 与共享资料固定引用；它不替代目标五件套或保存目标状态。
