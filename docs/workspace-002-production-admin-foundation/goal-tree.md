...
```text
GOAL-001-production-admin-foundation [active] 生产级可用 Admin 基架 (3/5)
...
└── GOAL-007-r4-schema-crud [active] R4 · Schema 驱动 CRUD 与 SQLite 持久化闭环 (0/6)
```
...
|----|------|--------|--------|----------|---------|
| `GOAL-001-production-admin-foundation` | 生产级可用 Admin 基架 | `null` | `active` | `3/5` | 2026-08-02 |
...
| `GOAL-007-r4-schema-crud` | R4 · Schema 驱动 CRUD 与 SQLite 持久化闭环 | `GOAL-001-production-admin-foundation` | `active` | `0/6` | 2026-08-02 |
...
> Root D-010 已关闭 `I-004` 的方向/立项目门禁；D-011 已创建 `GOAL-007-r4-schema-crud`（`active / 0/6`）。其 `I-007-001`～`I-007-004` 均为 open required，分别阻断受影响的精确契约、SQLite 持久化、Schema 写交互与重启验收；因此 Root R4 仍未勾选，Root 保持 `3/5`。
