...
| `I-005-004` | R2 最小 CORS / 同源托管假设是什么？ | non-blocking | R2 验收边界 | R2 验收前 | 记录 R2 所需最小假设；完整部署基线属 Root `I-005` | open | R2 验收前复核 | 待收集（Bearer 方案不依赖同源 cookie） |
| `I-005-005` | 与 R3 依赖边界：认证产物身份对象与 R3 用户—角色模型字段是否兼容？ | non-blocking | R3 平滑扩展 | R2 验收前 | 确认身份对象字段边界，不实施 R3 | open | R2 验收前复核 | 待收集 |
