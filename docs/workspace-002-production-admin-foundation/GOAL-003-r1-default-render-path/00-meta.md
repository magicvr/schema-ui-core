...
- [x] 既有 5 个手写示例**迁移为 Schema 文档**，经默认 Schema 主路径渲染；应用内不再存在手写示例作为独立页面路径（D-003）—— 5 份文档由 GOAL-004 落地（2026-08-01），手写示例源文件已清理
...
`progress: 4/4` 由上方 4 条成功标准等权派生；标准 2 于 2026-08-01 随 GOAL-004 落地 5 份迁移文档并经默认主路径渲染后闭合（本目标已移除手写默认分支，源文件清理见 02-execution）。
...
| `I-003-002` | table 数据注入（现由示例拥有）在默认路径如何挂接？ | required | 列表页可渲染 | 默认路径验收前 | 与 GOAL-002/004 对齐；可先静态/既有 records API | closed | 与 004 联调复核 | GOAL-004 **D-004** + `SchemaTable` 默认注入（reuse `GET /api/records`，读 table 节点 columns/dataSource）；App 主路径已接入，2026-08-01 验证全绿 |
