...
- [ ] **S1 · 版本迁移与可恢复起点**：建立 `schema_migrations`、顺序/校验和检查、事务化 fail-closed 迁移，并在升级前产生可恢复数据库副本。
- [ ] **S2 · 规范化 RBAC 与两步兼容**：建立角色/权限/用户关联/菜单 grant 表，回填并双读核对 `users.roles`；切换规范化读写后仍保持 R2 身份与 refresh 契约，旧列不在本步删除。
...
`progress: 0/6` 由上方六个顺序检查点等权派生。检查点只在实现事实与对应验证齐备后勾选；不得用计划、局部测试或百分比替代门禁和审计。
