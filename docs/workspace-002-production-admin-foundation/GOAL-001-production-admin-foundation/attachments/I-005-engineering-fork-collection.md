...
> **裁决状态**：用户于 2026-08-02 按 P-004 书面裁决附件 §6 四类边界——**部署基线 A**（文档双进程 + 可选 Docker Compose）、**建议计时口径**（终点=登录+后台首页可交互、不含依赖下载、≥1 次独立复现）、**建议复现方法**（文档步骤 + smoke 清单 + 独立复现记录，R5 落地 `scripts/smoke.sh`）、**I-006 方案甲**（操作日志为 R5 可选加分 checkpoint）。Root **D-013** 已将 `I-005` 置为 `verified`、`I-006` 置为 `closed`。
...
|------|------|------------------------|------|
| **A · 双进程文档 + 可选 Docker Compose 一键启动（推荐）** | 本地双进程为默认（现状命令 + 文档补齐）；另交付 `compose.yaml`：api 多阶段构建（Go → 精简镜像，暴露 :8080，DB volume）+ web 静态构建（nginx/Caddy 服务 `dist/` 并把 `/api` 反代到 api service），单源同源、免 CORS | 直接满足 #7「Docker 一键启动」与 #6「按文档配置并启动」；健康探针复用 `/healthz` | 中：需 compose + 镜像 + 文档；web 反代与 api 探针有现成基础 |
...
> 建议：**A**。B 会把 VP-002 #7 的 Docker 一键启动直接留在 R5 未交付；C 需要改动 api 主路径（与「Schema Renderer 主路径不改写」目标交叉）。A 保留现有本地路径、新增可选容器路径，最贴近 VP-002 成功边界。最终形态、镜像方案与是否纳入 R5 由用户裁决。

## 3. 15 分钟 fork 计时口径（待用户裁决）
...
- **裁决点**：甲（纳入，作为可选加分 checkpoint）还是乙（本波次排除）。升级为 required 需另记决策并更新 VP/Root 信息表。

## 6. 待用户裁决点（P-004 · 信息就绪门禁）

> **已裁决（2026-08-02 · Root D-013）**：① 部署基线 = **A**；② 计时口径 = **建议口径**；③ 复现方法 = **建议方法**（含 R5 落地 `scripts/smoke.sh`）；④ I-006 = **方案甲**。`I-005` → `verified`，`I-006` → `closed`。下列「候选/建议」保留为 R5 子目标方案冻结的输入，不再作为开放裁决点。

`I-005` 在下列裁决完成前保持 `collecting`（required），继续阻断 R5 方案冻结与立项：

1. **部署基线**：选 A（文档双进程 + 可选 Docker Compose）/ B（仅文档双进程）/ C（API 托管静态单进程）？（建议 A）
2. **15 分钟 fork 计时口径**：终点 = 登录 + 后台首页可交互？是否含依赖下载？是否要求 ≥1 次独立复现？（建议：不含依赖下载；终点登录+首页；记录一次完整复现）
3. **可复现实验方法**：采纳「文档步骤 + smoke 清单 + 一次独立复现记录」口径？是否在 R5 落地 `scripts/smoke.sh`？（建议采纳；脚本在 R5 子目标实施）
4. **I-006 最小操作日志**：方案甲（R5 可选加分 checkpoint）/ 方案乙（本波次排除）？（建议甲）

裁决后：由 `/govern` 记录 R5 方案决策，判断 `I-005` 是否可置 `verified`，再进入 R5 子目标立项。R1～R4 结论与本收集互不推翻。
