...
- [ ] **R4 · Schema 驱动 CRUD 与统一交互闭环**：以代表性实体验证列表、表单、操作、校验、加载/空态/错误态及权限失败。
  阶段子目标：`GOAL-007-r4-schema-crud`（**active / 0/6**，2026-08-02；D-010 冻结 records + SQLite + 重启保持 + 统一错误 envelope；D-011 建立端到端目标）。目标内 `I-007-001`～`I-007-004` 均为 open required，分别阻断精确 API/错误契约、SQLite 迁移/seed/并发、Schema 写交互与重启验收的受影响范围；R4 尚未实施或验收。
