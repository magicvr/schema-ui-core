...
parent: null
version: 0.2.0
## D-014 · A-002 响应：缺陷修复与通用适配层拆分

- **日期**：2026-08-03
- **状态**：accepted
- **用户裁决**（P-004，对 A-002 independent · fail 的三条 required 与 recommended）：
  1. **F-002-001（Renderer 硬编码 records 实体）**：走「通用适配层改造」关闭路径（`fixed`），不降级 VP-002 主张。
  2. **F-002-002（表单校验错误不阻断提交）** 与 **F-002-003（认证失效状态不清理）**：走 `fixed` 路径。
  3. **F-002-004~006（recommended）**：纳入 GOAL-009 可选加分（是否实施待用户决定）。
  4. **A-002 同 scope self 审计**：延后至修复完成后随关门补（P-004 §3.1）。
- **决定**：
  1. 新建 `GOAL-009-a002-auth-form-fixes`（active 0/4）承载 F-002-002/003 修复与回归。
  2. 新建 `GOAL-010-a002-schema-adapter`（active 0/5）承载 F-002-001 通用适配层改造（P-001 路线图 S1～S5；实施前 required `I-010-001`/`I-010-002`）。
- **理由**：F-002-001 是范围/架构决策且工作量大，独立目标 + 路线图承载，避免把大改造与小缺陷修复混在单一验收口径下；F-002-002/003 是确定性小缺陷，独立目标便于快速修复、回归与关闭证据复核。降级路径会弱化 VP-002 成功标准 6「Fork 后可直接接业务」的核心主张，故不采用。
- **未选方案**：
  - **降级为单一 records 示例声明**：需收窄 VP-002 表述并重新裁决，违背 VP 产品意图。
  - **合并单目标承载全部响应**：混合大改造与小修复，验收与关门口径混杂。
  - **不建子目标直接在 Root 层修复**：缺少波次载体，Root finding 关闭证据无落点。
- **影响**：Root A-002 F-002-001~003 保持 `open`（载体目标实施完成并有证据后按 `fixed` 闭合）；**Root 关门与 VP-002 关门继续被 A-002 开放 required 阻断**；Root `status: active`、派生进度 `5/5` 不变。
- **后续**：优先实施 GOAL-009 S1/S2；GOAL-010 先收集冻结 `I-010-001`/`I-010-002`；两目标完成后经 `/audit` finding-closure 复审，再进入 Root close-out 流程。
# 决策 · GOAL-001
...
## D-015 · 响应 A-005：回退 Root 关门并立项 GOAL-012

- **日期**：2026-08-04
- **状态**：accepted
- **用户裁决**：独立审计发现阻断问题 → 工作区 2 新设子目标修正 + 回退 Root 关门状态。
- **决定**：采纳 A-005 `fail` 与 F-001 required；F-001 走 `fixed`；创建 `GOAL-012-a005-shell-nav-fixtures`；Root `status: done → active`（progress 保持 `5/5`）。
- **理由**：P-003 未合法闭合 required 时不得维持 `done`；死链属于产品面阻断而非文档口误。
- **影响**：Root/VP-002 重新关门阻断至 F-001 闭合；纲领检查点不撤销（R1～R5 仍勾选）。
