...
五个检查点默认等权、原则上串行（P-001）。当前进度 `3/5`（S1/S2/S3 已完成；S4/S5 未实施）：
...
`progress` 由 S1～S5 五个检查点等权派生；当前为 `3/5`。检查点不替代审计、信息门禁或关门结论。
...
| `I-011-003` | 双资源验收如何证明“前端 Schema-only 接入”以及 fresh fork/升级/重启/401/403 的完整边界？ | required | S4 集成验收与 S5 关门 | S4 首个验收实现前 | 冻结 users/roles 两套验收矩阵、Renderer diff 边界、数据库升级夹具与 E2E/restart 协议 | **open** | 不延期；S4 前冻结 | 待确认；不以通用工厂单测替代真实双资源产品证据 |

开放 required 信息项只阻断其列明的阶段；当前 S1～S3 已完成，`I-011-001`/`I-011-002` 已 verified，`I-011-003` 到期（S4 首个验收实现前）待冻结。
