...
|--------|------------------|------|
| workspace 绑定 | **pass** | `workspace.md`：`id=workspace-003-…`，`root_goal=GOAL-001-…`，`canonical_scope=docs/workspace-003-…/`，`vision_role=delivery`，`plan_refs`/`primary_plan=VP-003-modular-admin-architecture`，`shared_materials_catalog: none` |
