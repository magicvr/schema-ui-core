...
| F-003 · Schema 非 ContributionSet 驱动 | `fixed`：`handler.RegisterSchemas` 接受 runtime `set.Pages` 派生 owner；composition 传贡献；贡献页 + 启用模块门禁（提交 `d1c372e`）；GOAL-012 A-003 |
...
F-008（可见性）与 F-003（Schema 贡献驱动）已闭合；F-004/F-007 部分闭合（R6 删除
清单）。F-001/F-002/F-005 保持 `open required` 但**可见**于 R5-I001（GOAL-012），
VP 退出 #2/#3/#5 取证与 Root done 宣称在闭合前不得成立。R6 承接实现迁出。
