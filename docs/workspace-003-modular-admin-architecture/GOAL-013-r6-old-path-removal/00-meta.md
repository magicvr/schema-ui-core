...
- [ ] **C6.2 / Persistence 所有权迁出**：store 上帝对象拆分（平台 runner/ledger vs
  模块仓储）；`CollectPersistence` 生产接线 + 0001-0008 descriptor 归属；
  seed/RBAC reconcile 以 Authorization/system-data 贡献为源。
...
四个检查点等权；当前 `progress: 0/4`。完成本子目标表示 R6 关闭；Root close-out 与
VP-003 关门另需确认。
...
|------|------|----------------|------|----------|----------|------|------|
| R6-I001 | required | 旧装配双轨清单与删除证据？ | C6.1 | C6.1 | 全仓扫描 + 删除 | collecting | R5 E-005 residual |
| R6-I002 | required | store·Persistence 所有权模型与 CollectPersistence 接线边界？ | C6.2 | C6.2 | 设计 + 实施 | collecting | Root A-010 F-001/F-002 |
...
1. 旧路径删除清单 + 执行（C6.1）。
2. Persistence 所有权迁出 + CollectPersistence 接线（C6.2）。
3. Schema 字节贡献发布 + 收尾（C6.3）。
4. 完整回归 + VP 退出 #1-#7 逐条取证 + self + Grok + Root/VP 关门（C6.4）。
