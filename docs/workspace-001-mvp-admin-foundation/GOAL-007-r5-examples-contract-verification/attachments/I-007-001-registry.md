...
| D-DATA | include | `search-form-table` / `data-table`（Go 列表/详情 + 前端） | `page` schema | fixtures `request-construction`、`response-mapping`、`query-serialization`、`static-data` | **已实现（批次 2a）**：Go `GET /api/records`（q/sort/order/page/pageSize）+ `GET /api/records/{id}`（`apps/api/internal/handler/records.go`）；Web `fetchRecords`/`buildRecordsQuery`/`parseRecordList`（`apps/web/src/renderer/records.ts`）+ `useRecords` hook（`use-records.ts`）；范例页 `search-form-table` / `data-table`（`apps/web/src/app/examples/`）。新增 Go 测试 6 项、Web 测试（records 11 项 + app-examples 3 项）入账；浏览器实测（Edge headless）两页渲染真实数据 | 
...
| 入口 | 命令 | 覆盖域 | 证据 / 说明 |
|------|------|--------|-------------|
| Web 单元 + upstream fixtures | `cd apps/web && npm test` | D-APP、D-PERM、D-EXPR(子集)、D-DATA、D-TABLE | 9 测试文件 / **114 项**（含 `upstream-fixtures.test.ts`、`permissions-inheritance.test.ts`、`app-manifest.test.ts`、`navigation.test.ts`、`App.integration.test.tsx`、`account/context.test.ts`、`data-table.test.tsx`、`records.test.ts`、`app-examples.test.tsx`）；上游 fixture SHA 与 provenance 已 pin |
| Web 构建 | `cd apps/web && npm run build` | D-APP、D-DATA、D-TABLE | tsc + vite build |
| Go 测试 | `cd apps/api && go test ./...` | D-PERM、D-VER(子集)、D-DATA | `internal/account/permission_test.go`、`internal/handler/account_test.go`、`internal/handler/records_test.go`（**21 项**）、`health_test.go` |
| Go 构建 | `cd apps/api && go build ./...` | D-PERM、D-VER(子集)、D-DATA | server 可运行 |
| Go 数据 API 冒烟 | `curl http://127.0.0.1:8080/api/records`（q/sort/order/page/pageSize）、`/api/records/{id}`、404 | D-DATA、D-TABLE | 批次 2a 运行时冒烟（list/detail/排序/分页/搜索/404）；详见 02-execution |
| 浏览器渲染 | 起 `apps/api` + `apps/web` 后访问 `/data-table`、`/search-form-table` | D-DATA、D-TABLE、D-APP | Edge headless 实测两页渲染真实数据（Acme Console…8 records），无 Loading/Error |
...
- P1 候选：`search-form-table` / `data-table`（D-DATA、D-TABLE）→ **已实现（批次 2a）**；`admin-list-edit-lifecycle` / `list-detail`（D-ACT、D-DATA、D-FORM）→ 登记为规划（批次 2b）。
- P2 候选：`form-with-reactions`（D-EXPR、D-FORM）→ 登记为规划（批次 2c）。
...
| D-DATA / D-TABLE（批次 2a 新增） | `apps/web/src/renderer/{records.ts,use-records.ts}`、`components/data-table.tsx`、`app/examples/{data-table-page,search-form-table-page}.tsx`、`apps/api/internal/handler/records.go` | `npm test`（records 11 项 + data-table 6 项 + app-examples 3 项）+ `go test ./...`（records 6 项）+ 浏览器渲染实测 |
