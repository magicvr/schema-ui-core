...
3. **R4 实施**：未开始；实施计划、代码与 fixture 对照测试按 02-execution 路线图推进；方案冻结不自动放行实施。
...
- R4 实施阶段：实施计划、`$context` 会话方案（静态/注入 vs token）、前后端实现、fixture 对照测试与运行时证据均须在 02-execution 记录；不把「方案已冻结」写成「已实现」。
