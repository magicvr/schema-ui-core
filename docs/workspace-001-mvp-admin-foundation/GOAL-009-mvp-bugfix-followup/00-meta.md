...
- [ ] **F-009-001**：`PATCH /api/records/{id}` 成功后刷新 `updatedAt`；`go test` 覆盖
- [ ] **F-009-002**：list-edit 使用真实 account `$context`；page 上存在可失败权限表达式；至少一条拒绝路径可测
- [ ] **F-009-003**：account 加载失败可观察（UI 或明确 prop/状态），不再静默丢弃 error
- [ ] **F-009-004**：`sessionProvider` nil 行为与注释一致（回落或改注释 + 防误用）
- [ ] **F-009-005**：`apps/api` 与 `apps/web` README 与当前端点/模型一致

> recommended（F-009-006 路由鉴权、F-009-007 body/pageSize 上限）默认纳入实施建议；若用户书面 residual，须在决策/审计留痕后可不阻断关门。

## 派生进度展示

显式检查点 = 上方 5 条 required 成功标准（等权）。  
当前 **0/5** → frontmatter `progress: 0/5`。progress 不放行、不推导 `done`。
...
| 2 | 修 F-009-001（API `updatedAt`） | 未开始 |
| 3 | 修 F-009-002（list-edit context + 权限演示） | 未开始 |
| 4 | 修 F-009-003 / F-009-004 | 未开始 |
| 5 | 修 F-009-005（README）；处理 006/007 或 residual | 未开始 |
| 6 | 回归 `go test` / `npm test` / 必要手测 + 阶段或关门审计 | 未开始 |

## 信息就绪与未知项（P-005）

| ID | 级别 | 所需信息 / 问题 | 影响门禁 | 最晚需要阶段 | 验证 / 收集动作 | 状态 | 延期 / 复核 | 证据 / 结论 |
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| I-009-001 | non-blocking | F-009-006（records 挂 `Allow`）是否纳入本目标必做？ | 验收范围 | 方案/实施前 | 用户确认 include 或 accepted-residual | open | — | 默认 recommended；见附件 §3 |
| I-009-002 | non-blocking | F-009-007（body/pageSize 上限）是否纳入？ | 验收范围 | 实施前 | 用户确认 | open | — | 默认 recommended |
...
- Root 纲领 R1–R6 与 `progress: 6/6` **不**因本目标改写；本目标自有 `0/5` 检查点。
