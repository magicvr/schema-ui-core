...
## D-015 · 立项 R6 集成验收与 VP 证据（2026-08-01）
...
1. 按用户 `/govern 规划 R6 — 集成验收与 VP 证据` 的明确指令，创建 `GOAL-008-r6-integration-acceptance-vp-evidence`，状态 `active`、处于规划期。
2. Root 路线图 R6 自「未开始」改为「规划中」，由 GOAL-008 承载四阶段路线：验收合同冻结 → 集成验收执行 → VP 证据汇编/整改 → R6 关门审计与 VP 提案输入。
3. 将 Root 成功标准中既有的 R6 职责显式化为未完成检查项；R6 仍是同一个第六纲领检查点，因此 `progress` 保持 **5/6**。
4. GOAL-008 登记 `I-008-001`～`I-008-005` required 信息项；计划冻结、验收执行、关门与 VP 提案受各自门禁约束。
5. 本回合只规划：不修改 `apps/*`、不执行 R6 验收、不改 VP status、不标 Root `done`、不主张完整协议支持。

**为什么**：

- R1-R5 已完成，R6 是 Root 唯一未完成纲领阶段；VP-001 的退出证据必须落在挂接工作区目标记录中。
- 当前证据分散在 R2/R3/R4/R5，且缺少对 VP 三条退出判据的集中映射、干净启动/浏览器验收合同、机器可读 evidence index 与平台范围决定。
- R6 横跨多个可独立验收块，按 P-001 应先立项并冻结高层路线/信息门禁，再执行；不能用既有 395 项测试或 Root `5/6` 自动放行。

**未选方案**：

- **在 Root 中直接堆叠 R6 执行流水**：不利于形成独立 close-out 与 VP 可链接的有界目标证据。
- **先改 VP 为 closed 再补证据**：违反 alignment 的工作区证据、Goal 关门与用户确认顺序。
- **把 CI、发布或部署静默设为必需或静默排除**：最低环境矩阵属于 `I-008-005`，需证据/决策后冻结。
