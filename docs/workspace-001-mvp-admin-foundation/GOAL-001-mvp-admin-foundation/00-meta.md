...
- [x] R6 已对照 VP-001 三条退出判据完成集成验收、证据汇编与关门审计，并形成工作区内可复核的 VP 提案输入。（GOAL-008 完成：A-005 self + A-006 independent close-out 均 pass、开放 required=0；`evidence-index.json` 7 artifact verified；[vp-evidence-assembly.md](../GOAL-008-r6-integration-acceptance-vp-evidence/attachments/vp-evidence-assembly.md) 三判据 Q2 映射）
...
| R6 | 集成验收与 VP 证据 | **完成** | GOAL-008 `done`（A-005 self + A-006 independent close-out 均 pass，开放 required=0；`I-008-001`～`I-008-005` verified）；用户 `/govern` 授权（Root D-016）；Root `progress` 6/6、status `done`；VP-001 关门另走 `/vision` |
...
| I-PROTO-003 | required | 每条纳入能力的范例页路径与自动化/手工验证入口？ | R5 验收 / 关门 | R5 验收前 | 为纳入域登记范例路径与验证命令/步骤 | **verified** | 2026-08-01 Root D-013 / GOAL-007 D-011 + A-008 | 登记表 v0.8.0 + 阶段 3/4 证据（`npm test` 395）；诚实边界见 D-013（reactions 上游排除、batch Q1） |
...
- R6 关门（2026-08-01）：GOAL-008 四阶段完成，A-005 self + A-006 independent close-out 均 **pass**、开放 required=0、`I-008-001`～`I-008-005` verified；用户 `/govern` 响应 A-006 并授权（Root **D-016**）：GOAL-008 → `done`、R6 → 完成、`progress` 5/6 → **6/6**、Root status → **`done`**（goal-tree 同步）。VP-001 随后经 `/vision` **closed**。R6 `pass`/Root `done` 不等于完整协议支持或发布就绪。
- 关门后修正跟随（2026-08-01）：用户立项子目标 [`GOAL-009-mvp-bugfix-followup`](../GOAL-009-mvp-bugfix-followup/00-meta.md)（`active`，`progress: 0/5`），承接 `apps/api`+`apps/web` 代码审视 required findings；附件 [audit-code-review-bugs-2026-08-01.md](../GOAL-009-mvp-bugfix-followup/attachments/audit-code-review-bugs-2026-08-01.md)。**不**改写本 Root 纲领 6/6 或 `status: done`；**不**重开 VP-001 范围。
