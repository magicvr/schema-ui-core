...
```text
GOAL-001-admin-module-readiness [active] (5/6)
...
|----|-------|--------|--------|----------|---------|
| GOAL-001-admin-module-readiness | Admin 业务模块准入与基架收敛 | null | active | 5/6 | 2026-08-10 |
...
- `progress: 5/6`（Root）仅由 Root 中显式列出的 S0–S5 六个阶段检查点派生，不放行阶段、不关闭 finding，也不代表 `go`。
