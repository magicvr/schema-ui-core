...
从 [roadmap.md](roadmap.md) v0.98.0 整理迁入。这里保留完整 VP 索引、架构能力编号、冻结合同和历史闭合证据；当前组合焦点与未决事项只在[主路线图](roadmap.md)维护。本文不建立 Goal 状态或审计权威；现行 VP 状态以 plans、执行与审计以对应工作区为准。

导航：[VP 索引](#vp-完整索引) · [架构清单](#架构分支) · [冻结合同](#架构已冻结合同) · [Admin 历史](#admin-能力与交付依据) · [已闭合登记](#已闭合事项留档) · [本地扩展](#本地扩展清单)

## VP 完整索引

按 VP 编号排序，编号不代表执行先后；VP-006 先于 VP-005 交付。各行保留范围、前置、lead、版本与关门依据；历史阶段条件不能被理解为当前开放门禁。VP-009/010 的波次数字仅为当时快照，现行波次以对应工作区 goal-tree 为准。

| 编号 | VP | 意图 | 前置 / 边界 | 状态与历史依据 |
|------|----|------|-------------|----------------|
...
| 40 | [VP-040-timestamptz-persistence-contract](plans/VP-040-timestamptz-persistence-contract.md) | 架构 · **C1 DB 时间列 timestamptz 持久化合同**（`RES-T03-tz` / `RT-T03`）：PG `timestamptz(6)` + SQLite fixed-6 UTC RFC3339 `TEXT`，全部绝对时刻列进入分母，双方言各自原地转换；公共时间 wire 统一 6 位微秒 RFC3339 UTC `Z`；C3 建立统一 Backup SPI/Service；不引入 ORM/第三库或 SQLite→PG 产品搬运器。 | 继承 VP-013 双方言端口/checksum 台账、VP-020 展示/输入时区合同、VP-035 评估；VP-039 波次已 `closed`；与 VP-009/010 正交；不消耗 A3/Redis/MQ | **closed**（2026-09-21 · v0.3.0；VRev-105 self `pass`；lead `workspace-040-timestamptz-persistence-contract`；Root `GOAL-001-timestamptz-persistence-contract` `done · 3/3`；R1/R2/R3 与六条退出判据完成；开放 required = 0；用户确认关门） |

## 架构分支

> 性质：P-005 能力清单；已交付波次各自冻结分母，剩余候选不因登记而自动立项。  
> **已冻结（2026-08-20 用户确认）**：Store 双方言决策，见[冻结合同](#已冻结store-双方言rt-p03)。  
...
| RT-T03 | 时区在持久化层的合同 | locale 在 Admin 功能面；DB `timestamptz` 合同已交付 | **delivered** | [VP-040](plans/VP-040-timestamptz-persistence-contract.md) `closed` v0.3.0（2026-09-21）；90 列 / 44 表合同分母、PG `timestamptz(6)`、SQLite fixed-6 UTC RFC3339 `TEXT`、双方言迁移与 wire/restore 验收均有 Root 证据 |
...
### 架构分支序列（2026-09-10 经 VP-035 R4 重述；已交付序列 + A0–A7 序列中唯一未交付的门控项）
...
A3  仅当需要多实例：就绪探针扩依赖、再评估 PG 锁/SKIP LOCKED                 [trigger-gated · A0–A7 序列中唯一未交付的门控项]
...
C1  DB 时间列 timestamptz 持久化合同（RES-T03-tz）
    — 现状：首波 90 列 / 44 表绝对时刻列已完成双方言迁移；非时间 INTEGER 显式排除（证据见 workspace-040 Root）。
    — 承接 = [VP-040-timestamptz-persistence-contract](plans/VP-040-timestamptz-persistence-contract.md) `closed` v0.3.0
      （lead = workspace-040-timestamptz-persistence-contract；Root `done · 3/3`；六条退出判据满足；VRev-105 `pass`）
```

**刻意后置**：MongoDB、ORM、Redis、消息队列、搜索引擎、K8s、SMS。它们是部署或产品触发的后果，或已否决的技术选型。

架构分支最近一拍（已交付并关门）：**[VP-040-timestamptz-persistence-contract](plans/VP-040-timestamptz-persistence-contract.md) `closed` v0.3.0**（2026-09-21 · VRev-105 self `pass` · lead `workspace-040-timestamptz-persistence-contract`；Root `GOAL-001-timestamptz-persistence-contract` `done · 3/3`；R1/R2/R3 全部完成）。前一拍 **VP-035-foundation-architecture-health `closed` v0.3.0**；A3 余项仍 trigger-gated。

## 架构已冻结合同

这些合同仍约束后续扩展；“不进本波”描述各历史 VP 的边界，不表示其他 VP 未交付该能力。

### 已冻结：Store 双方言（RT-P03）

2026-08-20 用户确认时尚未立项；其后由 VP-013 于 2026-08-21 交付并关门。下表保留已冻结决策。
...
用户确认（2026-08-24）：否决同日 SMTP 专用有界关门；实施史不回退。当时重开 lead `workspace-017-outbound-mail`；同日按现行渠道分母再次关门，Root `done 8/8`、VP-017 `closed` v0.5.0（VRev-042）。
...
| 内嵌默认 | **mock 站内出站记录**（管理员可检视）。无生产渠道仍能开发与快测。历史 `CaptureSink` 实施保留；R5/R6 后已交付管理员可检视的 mock 站内出站记录，具体实现见 VP-017。 |
...
VP-011 已交付的标准 Admin 模块（用户/角色/设置/钱包演示面等）不重开。下文保留原能力编号与交付依据；当前开放项统一见主路线图。
...
**基架能力剩余（原编号；已交付项保留供历史引用）**

1. 密码策略、邀请、账号恢复状态机——VP-019 已交付并关门（自助恢复 **硬前置** = VP-017 + 账号邮箱；邀请仍可用管理员出示链接）  
...
3. 配置包导出、diff、dry-run、导入 —— 已由 **[VP-025-config-export-diff-dryrun-import](plans/VP-025-config-export-diff-dryrun-import.md)** 承接并于 2026-08-30 **`closed`**（v0.3.0 · 用户书面确认 · roadmap 明文点名「当时登记的后续方向为 基架能力剩余 #3」）  
...
全局搜索 / Command Palette、Saved Views、批量结果中心、未保存保护、统一 Toast/错误恢复、版本与维护提示。全局搜索若需要专用引擎，拉动架构 RT-X01。**收口进度（2026-09-19）**：Command Palette = VP-036 `closed`；Saved Views / 未保存保护 / 统一反馈 = VP-037 `closed`；批量结果中心 = VP-038 `closed` v1.0.0；**版本与维护提示 = [VP-039](plans/VP-039-version-maintenance-diagnostics.md) `closed` v0.3.0**。上述有界体验波次均已关门；实体全文检索仍未实现，继续 trigger-gated（不等同于 Command Palette）。

**历史交付（VP-038）为 [VP-038-batch-operations-and-job-center](plans/VP-038-batch-operations-and-job-center.md)**（2026-09-19 激活并**同日关门** · **`closed` v1.0.0** · 用户书面确认 · Root `GOAL-001-batch-operations-and-job-center` `done · 5/5` · 判据 1～7 达成 · R5 cross 审计 self `A-001` + grok-build independent `A-002` 均 `pass` · 补做关门 Vision Review = [VRev-100](reviews/VRev-100-vp038-batch-operations-and-job-center-closeout.md)）：把 VP-012 显式排除的「通用 Job 管理页」与本节未立项的「批量结果中心」收口为有界产品能力——已注册 Job 种类的可见性与结果读取 + **至少一条**真实批量操作以异步 Job 承接 + 结果中心体验；实体全文检索、Saved Views 重做、新业务域、组织/权限域与 Redis/MQ/多实例不进入本波。**计划 self = VRev-098 `pass` · 激活 self = VRev-099 `pass`（0 required）**；lead `workspace-038-batch-operations-and-job-center`（Root `GOAL-001-batch-operations-and-job-center`），2026-09-19 用户确认激活并由 `/govern` 开区。`I-038-004` 用户 P-004 裁决 = 新建 `admin.jobs` 进 admin 默认集（**Profile 内容扩展，不改装配语义，不暂挂 `go`**）；Admin 类 freshness `0c29c08` → `7e5ce891` **PASS**。

历史交付（VP-037）为 **[VP-037-admin-workflow-continuity](plans/VP-037-admin-workflow-continuity.md)**：把已注册列表页的视图复用、离开前安全保护、成功/失败反馈与列表视觉/筛选体验收敛成一组可验证的工作流连续性能力；实体全文搜索、批量结果中心、组织/权限域与新业务域不进入本波。delivery workspace = `workspace-037-admin-workflow-continuity`；R1～R6 全部完成，Root `done · 6/6`，**2026-09-18 `closed` v1.7.0**（用户书面确认 · VRev-096 self `pass`）。

IAM 与格式能力的历史交付链：**[VP-019-iam-recovery](plans/VP-019-iam-recovery.md)（IAM：密码策略 / 邀请入职 / 自助恢复状态机）——2026-08-25 激活并同日全链交付，2026-08-26 `closed` v0.3.0（用户书面确认；Root done 4/4；关后 A-001/A-002 pass）**；硬前置 = VP-018 已校验邮箱（已 `closed` v1.0.0）+ VP-017 运输（已按现行分母再 `closed` v0.5.0）。不要把恢复状态机打进 VP-018。后续已交付：**[VP-020-timezone-number-currency-formatting](plans/VP-020-timezone-number-currency-formatting.md) `closed` v0.3.0**（2026-08-26 激活并开区 · 2026-08-27 关门 · 时区/数字/货币格式语义，基架能力剩余 #5 交付完成；lead `workspace-020-timezone-number-currency-formatting` 结项；关门审计双腿 pass）；当时登记的后续方向为 配置包导出/diff/dry-run/导入（基架能力剩余 #3 · **已由 [VP-025](plans/VP-025-config-export-diff-dryrun-import.md) 交付并 `closed`**）与体验增强（全局搜索 / Command Palette 等）；组织/部门/岗位 + 数据权限 `org`（#2）已于 2026-08-29 按用户指示降权为 **trigger-gated**（见「基架能力剩余」）。

截至 2026-09-21，Admin 最近关门：**[VP-039-version-maintenance-diagnostics](plans/VP-039-version-maintenance-diagnostics.md) `closed` v0.3.0**（2026-09-19 激活并关门；用户书面确认；VRev-103 self `pass`；workspace-039 Root `done · 4/4`；R1–R4 全部 done；Goal cross required=0；existing fresh-seed harness bounded residual 保持登记）。其前一拍 = **[VP-038-batch-operations-and-job-center](plans/VP-038-batch-operations-and-job-center.md) `closed` v1.0.0**。当前 Admin 功能分支无 active 交付 VP；文件扫描/隔离策略与组织/部门/岗位仍未立项或 trigger-gated。

> 2026-09-18 历史投影依据：VP-037 为 **`closed` v1.7.0**；Root 为 **`done · 6/6`**，R5 `GOAL-006` 为 `done · 4/4`，R6 `GOAL-007` 为 `done · 8/8`；关门依据 = 用户书面确认（`GOAL-006` `D-002`，前置条件为整改子目标 `GOAL-011` 修正两个分页/文案缺陷）+ `VRev-096` self `pass`。仍开放项见[主路线图未决登记](roadmap.md#未决项统一登记残余--悬置--trigger-gated)。

## 已闭合事项留档

以下是原未决登记的闭合记录，保留事实、证据和复核约定，不再列为开放待办。
...
| DB `timestamptz` 持久化合同（C1 / `RT-T03` / `RES-T03-tz`） | **已关门**：[VP-040-timestamptz-persistence-contract](plans/VP-040-timestamptz-persistence-contract.md) `closed` v0.3.0（2026-09-21 · VRev-105 `pass`）；`workspace-040-timestamptz-persistence-contract` / Root `GOAL-001-timestamptz-persistence-contract` **`done · 3/3`**；R1/R2/R3 全部完成，六条退出判据 verified，`I-040-001`～`005` 均 verified；Goal independent 关门意见 `GOAL-008/A-002`（conditional/0）与 `A-005`（pass/0）；Vision open required = 0；用户确认关门 | 已交付 | 无后续门禁；后续新增工作依 `/vision` 结构选型另行处理 | roadmap 架构 C1；VP-040；Root `D-020`；`GOAL-008` 退出矩阵、`A-002`、`A-005` |

## 本地扩展清单

截至 2026-09-19，供“pinned vs 本地扩展”判断；清单仍为部分补齐，开放登记见主路线图。此处只列扩展合同，e2e 审计记录已归回未决或已闭合登记。
