...
- **现行组合投影（2026-09-17）**：当前 active 交付 VP = **[VP-037-admin-workflow-continuity](plans/VP-037-admin-workflow-continuity.md)**（v1.0.0；lead `workspace-037-admin-workflow-continuity`；Root `GOAL-001-admin-workflow-continuity` active · 4/5；R1～R4 已完成，R5 `GOAL-006-r5-composition-acceptance` active · 3/4；VRev-095 self `pass`）。最近关门 = **[VP-036-admin-command-palette](plans/VP-036-admin-command-palette.md)**（closed v0.3.0；lead `workspace-036-admin-command-palette`；VRev-093 self `pass`；Root `GOAL-001-admin-command-palette` done 4/4）。持续程序 VP-009/VP-010 与已关闭 VP 的历史绑定不变；不改变 Charter `primary_workspace`。
- **2026-09-18 组合投影（editorial）**：VP-037 仍为唯一 active delivery VP，计划 v1.1.0；Root `GOAL-001-admin-workflow-continuity` 为 `active · 5/6`，R5 `GOAL-006` 为 `active · 3/4`，R6 `GOAL-007` 为 `done · 4/4`。R5-I-004 用户书面关门确认仍开放，Root/VP 不关门；该投影不改变 Charter 0.4.0 的 strategic 目的、边界或非目标。
