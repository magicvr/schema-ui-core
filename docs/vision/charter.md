...
- **现行组合投影（2026-09-14）**：当前无 active 交付 VP；最近关门 = **[VP-036-admin-command-palette](plans/VP-036-admin-command-palette.md)**（closed v0.3.0；lead `workspace-036-admin-command-palette`；VRev-093 self `pass`；Root `GOAL-001-admin-command-palette` done 4/4）。持续程序 VP-009/VP-010 与已关闭 VP 的历史绑定不变；不改变 Charter `primary_workspace`。
