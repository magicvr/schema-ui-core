...
- **现行组合投影（2026-09-14）**：active 交付 VP = **[VP-036-admin-command-palette](plans/VP-036-admin-command-palette.md)**（v0.2.0；lead `workspace-036-admin-command-palette`；VRev-092 self `pass`；Root `GOAL-001-admin-command-palette` active 0/4）。持续程序 VP-009/VP-010 与已关闭 VP 的历史绑定不变；不改变 Charter `primary_workspace`。
