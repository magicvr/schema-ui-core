...
| 项 | 值 |
|-----|-----|
| status | **`closed`**（2026-09-21 · v0.3.0 · 1 个 delivery 区 · `workspace-040-timestamptz-persistence-contract`） |
...
| 激活门禁 | **已满足（2026-09-20）**：① [VP-039](VP-039-version-maintenance-diagnostics.md) 已 `closed`；② `I-040-001` 已由用户 P-004 选择 R1 方向；③ 架构类 freshness `6197e802` → `b0a6789b` PASS；④ 激活就绪 self Review [VRev-104](../reviews/VRev-104-vp040-timestamptz-persistence-contract-activation.md) `pass`；⑤ slug/Root 已由 `/govern` scaffold。 |
| 关门审视 | [VRev-105](../reviews/VRev-105-vp040-closeout.md) self `pass`（2026-09-21；六条退出判据满足；投影同步 finding 已 fixed；open required = 0） |
...
Admin 时区 / 数字 / 货币**展示与输入**已由 VP-020 交付。立项时，持久化层时间列仍普遍为 SQLite 兼容的 `INTEGER` epoch；VP-035 R2/R3 核对 `apps/api` 内 `timestamptz` 命中 0，因此将 `RT-T03` 交由本 VP 承接。该缺口不是符合性漏做（从未写入已交付分母）；本 VP 已交付并关门。
...
## 纲领路线图（R1/R2/R3 已完成；证据见 workspace-040 Root）

```text
R1 合同与分母冻结：方言物理类型、列清单、零值、备份 residual [completed · GOAL-002]
  → R2 双方言迁移 + Store 编解码 [completed · GOAL-003/004/005]
  → R3 读写/时区回归 + 备份有界核对 + 证据与关门 [completed · GOAL-006/007/008]
```
...
|----|--------------|------|----------|----------|------------------|------|-------------|-------------|
| I-040-001 | SQLite 用什么物理类型与 PG `timestamptz` 合同平等？（TEXT RFC3339 / INTEGER epoch+约定 / 其他） | required | 阻断 R1 冻结与 R2 迁移（已完成） | R1 | 对照 VP-013 历史合同与用户 D-002；核对 `timestamptz(6)` / fixed-6 RFC3339 TEXT 编解码 | **verified** | — | `GOAL-002/A-046`、`A-047`；90 列逐列合同与 `internal/temporalcontract` |
| I-040-002 | 哪些列进首波分母？全部绝对时刻列是否全覆盖？ID/duration/step/version/计数/金额/flag 如何排除？ | required | 阻断 R1/R2 | R1 | 全仓扫描 compiled catalog 与 runtime repository，形成逐列 inventory | **verified** | — | `GOAL-002` inventory：90 列 / 44 表；排除列清单及 PG leftover 守卫 |
| I-040-003 | 存量库升级策略与备份 residual？无产品搬运器是否再次声明？ | required | 阻断 R1 方案与判据 4 | R1 | 对照 VP-013/016 dump 路径；设计双方言原地转换与失败恢复 | **verified** | — | `GOAL-002/A-048`：`F-I-005` 按 `fixed` 闭合；双方言各自原地转换；无 SQLite→PG 产品搬运器 |
| I-040-004 | 与 VP-020 展示合同的回归矩阵（会话时区、UTC 存储） | required | 阻断 R3 | R1 | 复用 VP-020 验收用例，补存储形状对照 | **verified** | — | `GOAL-006/02-execution/E-004`：单位族、DST / +05:45 / 负 epoch、Go wire 与 Web 会话时区 round-trip |
...
|--------------|-----------|------|--------|-------|
| workspace-040-timestamptz-persistence-contract | GOAL-001-timestamptz-persistence-contract | delivery | 2026-09-20 | `done` · Root `done · 3/3`；R1/R2/R3 全部完成 |
...
|------|---------|---------|----------------|-----------|
| 2026-09-21 | closed | 六条方向级退出判据满足；Root `done · 3/3`，R1/R2/R3 全部完成；VRev-105 self `pass`，开放 required = 0；用户确认关门 | workspace-040 Root `D-020`；GOAL-008 退出矩阵；`GOAL-008/A-002`、`A-005` | 无开放 required 或接受残余 |
...
| 2026-09-21 | 用户指令按流程关闭 VP-040；workspace-040 Root `done · 3/3`、六条方向级判据满足，Vision open required = 0。self 关门审视 [VRev-105](../reviews/VRev-105-vp040-closeout.md) `pass`；VP-040 `active → closed` v0.3.0。 |
