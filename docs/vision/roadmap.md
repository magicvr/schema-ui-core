...
| 7 | [VP-007-localization-and-system-settings](plans/VP-007-localization-and-system-settings.md) | 建立 `zh-CN` / `en-US` 多语种运行时与 `auto` 解析，并把既有 Settings 产品化为 General / Branding / Localization / Appearance 四类系统设置。 | 继承 VP-003/004 模块边界、VP-005 设计系统与 VP-006 完整协议面；不改变双 Profile 的 Settings 可见性边界 | **planned**（2026-08-09 用户确认意图；0 区，待激活与 `/govern` 建 delivery 工作区） |
...
|------|------|------------|------------------|
| 8 | 订单、钱包、类目、通知等业务能力 | 默认承载：VP-003 架构 + VP-004 playbook + **VP-006 协议面** + VP-005 设计系统；可复用 planned VP-007 的 locale/settings 基础，但不得假装其未交付能力已经可用 | 建 VP 前须 `/vision` 复核；不得用业务模块倒逼恢复长期双线、跳过协议覆盖或预支 planned VP-007 的实现事实 |

**当前交付意图**：**无 active 交付 VP**。VP-001～006 均 **closed**（协议覆盖权威 `I-PROTO-FULL-001`）；[VP-007](plans/VP-007-localization-and-system-settings.md) 已由用户确认意图并以 **`planned`** 落盘，尚未激活、未绑定工作区，也不构成任何多语种或 Settings 新能力已交付。下一步须先由 `/vision` 确认激活与 lead delivery 工作区，再交 `/govern` 建 Root 与纲领路线图。订单/钱包/类目/通知仍是后续独立业务 VP 候选。
