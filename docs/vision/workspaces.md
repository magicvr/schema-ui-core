...
| workspace-017-outbound-mail | docs/workspaces/workspace-017-outbound-mail/ | GOAL-001-outbound-mail | delivery | VP-017-outbound-mail | active（2026-08-24 用户否决关门并重开；VP-017 `active` v0.4.0；Root `active` 4/8；R1–R4 子目标保持 `done`；渠道升级） |
| workspace-018-account-email-identity | docs/workspaces/workspace-018-account-email-identity/ | GOAL-001-account-email-identity | delivery | VP-018-account-email-identity | active · **冻结**（VP-018 `active`；Root `blocked`；直至 VP-017 再次关门不得推进） |
...
- **workspace-017（2026-08-22 开区）**：VP-017（出站邮件）唯一 lead delivery 工作区。R1–R4 已实施内核端口 + SMTP + capture（子目标保持 `done`，实施史不回退）。**2026-08-24：用户否决同日有界关门**；VP-017 `closed → active`；Root `done → active`；现行分母 = 渠道模型 + mock 站内记录 + Resend + 设置/试发。不改变 Charter `primary_workspace`。
- **workspace-018（2026-08-24 开区）**：VP-018（账号邮箱身份）唯一 lead delivery 工作区。同日用户要求 **冻结**：Root `blocked`；在 VP-017 按现行分母再次关门之前不得推进。不改变 Charter `primary_workspace`。
