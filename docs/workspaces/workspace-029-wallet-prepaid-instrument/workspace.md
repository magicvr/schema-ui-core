...
- **Root** `GOAL-001-wallet-prepaid-instrument`：`active` · **4/5**（R1～R4 done · R5 Admin 自助核销 [GOAL-005 active 0/4]），纲领见 Root `00-meta.md`。
...
VP-029：钱包预付资金凭证与外部主体接缝（vision_ref @0.4.0）——历史判据 #1～#7 已核销；现行追加 #8～#10（已登录自助核销 HTTP / 我的钱包入口 / 限流评估）。红线 = 不重开 VP-011、不改 Profile 默认集/Manifest、不引入支付网关或 Telegram、不把 Bot 用户写入 `admin.users`、Admin 自助核销不记入 subject 账。
...
| R5 | **Admin 自助核销 HTTP + 我的钱包入口**（判据 8/9/10） | **active**（GOAL-005 3/4；待 S4 independent） |
