...
- **Root** `GOAL-001-wallet-prepaid-instrument`：`active` · **2/4**（R1 合同冻结 [done] → R2 主体接缝+账本入金 [done] → R3 Admin 批次面+导出 → R4 证据与关门），纲领见 Root `00-meta.md`。
...
| R3 | **Admin 批次面 + 导出**（判据 2/5 + I-029-004）：生成/导出/作废/查询 · 权限键 · 操作审计 · 明文不进审计原文 | 待推进（立项 GOAL-003） |
