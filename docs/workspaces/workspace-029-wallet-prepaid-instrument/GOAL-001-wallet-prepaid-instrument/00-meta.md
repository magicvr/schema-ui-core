...
| I-029-004 | non-blocking | 导出格式（CSV/TXT、是否含明文、一次性下载）。 | 判据 5 | R3 | lead 建议 + 用户确认 | open | — | 待确认（R3 前置） |
| I-029-005 | non-blocking | C 端自助核销 HTTP 是否本波交付，或仅模块 API。若选 HTTP，本 VP 必须完成 RT-Q05 精神的限流评估，不得推到未激活的 VP-030。默认倾向：模块 API 必做，HTTP 可选。 | 判据面 | R1 | 用户裁决（R1 可确认默认） | closed | — | D-002：本波仅交付 Go 模块内部 API Redeem，暂不暴露公开 HTTP |
