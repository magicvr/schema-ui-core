...
- Root `GOAL-001-admin-command-palette`：**`active · 0/4`**；R1～R4 纲领阶段按序承接 VP-036。
...
| Root Goal | `GOAL-001-admin-command-palette` | `parent: null`；**active · 0/4** |
...
| 阶段 | 目的 | 状态 |
|------|------|------|
| R1 | 页面/导航/动作分母、Profile/权限语义、排序/去重、快捷键与实体搜索排除冻结 | not started；I-036-001～003 collecting，进入冻结前必须处理 |
| R2 | `SearchableItem` 契约与跨模块 provider 聚合 | not started；依赖 R1 |
