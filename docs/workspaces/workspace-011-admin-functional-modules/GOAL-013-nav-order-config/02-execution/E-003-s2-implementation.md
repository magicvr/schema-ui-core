...
- **UI 侧边栏顺序的真实载体是 manifest 聚合**（`manifest.ForModulesWithFragments`，按 ModuleID 字母序合并各模块 fragment 的 navigation 槽），**不是** kernel 的 `sortNavigation`（后者只保证贡献切片的确定性顺序；system-data reconcile 的 menu_items.sort_order 落库仍为模块声明 Order——A-003 F-001 用户裁决 accepted-residual：产品权威序 = 仅 manifest）。
- 因此排序需要在**两层**应用：kernel（系统数据顺序）+ manifest（发布文档顺序，UI 实际渲染）。D-001 原描述只覆盖 kernel 层；S2 补充 manifest 层。

## 实现内容

- kernel：`DefaultNavigationOrder` 常量（12 项，用户确认清单）；`Plan.NavigationOrder` 字段；`sortNavigation(nodes, order)` 三层排序（清单优先 → 未列 NodeID 追加末尾 → Parent 分组仍优先）；`NormalizeNavigationOrder` 未知 NodeID 整体回退默认 + 告警（kernel + manifest 共用）。
