...
| R3 | **常用波**（S-01～S-14）：数据字典、文件/附件库、系统监控与错误日志、定时任务、公告、API 令牌、类目、商品、数据权限（行级）、MFA/2FA、登录验证码、回收站/软删除、**订单、钱包/账务**（A-002 F-001 降档） | 依赖 R2 | 进行中（第一批次 GOAL-007 S-02 文件/附件库 + GOAL-008 S-01 数据字典已立项） |
