...
| R2 | **一等公民波**（F-01～F-04）：仪表盘/控制台、数据导入导出、个人中心与账户安全（含账号启停）、通知中心——每个纳入能力带协议驱动范例页 + 验证路径；立项时逐项核对 I-011-001 §8 方案必办 | 依赖 R1 | **立项完成**（GOAL-003～GOAL-006，0/5；S1 方案冻结待推进） |
| R3 | **常用波**（S-01～S-14）：数据字典、文件/附件库、系统监控与错误日志、定时任务、公告、API 令牌、类目、商品、数据权限（行级）、MFA/2FA、登录验证码、回收站/软删除、**订单、钱包/账务**（A-002 F-001 降档） | 依赖 R2 | 待立项 |
