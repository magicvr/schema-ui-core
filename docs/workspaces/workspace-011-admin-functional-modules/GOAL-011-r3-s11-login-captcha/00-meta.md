...
- [ ] **S1 · 方案冻结**：挑战类型（算术/图形/token）、与锁定限流的叠加语义、配置开关、协议对照；方案级 self 审视
...
|----|------|-----------------|----------|--------------|-----------------|------|
| I-001 | required | 挑战类型与生成/校验模型（无第三方依赖偏好） | S1 方案 | 自研算术/图形挑战 or 依赖评估 | open |
| I-002 | required | 与既有 423 锁定/限流的叠加语义与配置默认 | S1 方案 | auth 模块现状对照 | open |
| I-003 | required | 登录链路改动对 go（核心认证）的影响判定 | S4 | VP-008 接口对照 | open |
