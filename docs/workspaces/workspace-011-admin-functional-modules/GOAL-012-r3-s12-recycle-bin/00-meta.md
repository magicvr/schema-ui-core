...
- [ ] **S1 · 方案冻结**：受管资源范围、快照模型（payload/外键）、恢复/清除语义、与既有删除路径接入；方案级 self 审视
- [ ] **S2 · 实现**：回收表迁移 + 删除钩子接入 + 管理端点 + schema 页 + 测试
- [ ] **S3 · 验证**：单元/集成 + 恢复/清除实测 + 全量回归
- [ ] **S4 · go 影响判定 + 自审**（数据删除语义 → go 检查重点）
...
|----|------|-----------------|----------|--------------|-----------------|------|
| I-001 | required | 受管资源范围与快照模型（payload 可恢复性/外键依赖） | S1 方案 | 对照既有资源删除路径 | open |
| I-002 | required | 恢复/彻底清除语义（与外键/级联/审计的关系） | S1 方案 | 既有删除语义对照 | open |
