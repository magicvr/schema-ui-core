...
|----|------|-----------------|----------|--------------|-----------------|------|
| I-001 | required | cron 表达式校验器（依赖/自研）与运行语义（重叠/补跑） | S1 方案 | 评估 robfig/cron 依赖 or 自研校验 | **closed**（D-001 §5 / D-002 §2） |
| I-002 | required | 调度器边界：进程内 vs 持久化任务队列；重启语义 | S1 方案 | 单实例假设文档化 | **closed**（D-002 §3） |
| I-003 | required | Profile 归属：admin 默认集 | S1 方案 | 先例一致 | **closed**（D-001 §2） |
