...
- **A-016 判定 open、由 A-017 修正后仍留缺口的项**：F-012（`updated`/version 核账再次失配）、F-013（自检脚本 BOM 与检查 6 未比较 version）、F-014（Root 结论自相矛盾 + version 未按声明 bump）——**A-018 判其仍 open**；本响应（A-019）已逐条修正，闭环证据待下一次 focused independent re-audit（**A-020**）。
