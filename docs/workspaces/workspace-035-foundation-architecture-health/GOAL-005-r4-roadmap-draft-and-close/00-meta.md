...
- [ ] C4：VP-035 六条方向级退出判据逐条取证（判据 1～3 已由 R2/R3 交付；判据 4 = 草案已交 `/vision` 并经用户采纳；判据 5 = 边界未越；判据 6 = 开放 required = 0）。→ [判据矩阵](attachments/exit-criteria-matrix.md)：判据 1～5 达成，判据 6 待审计。
...
进度由五项等权计算；当前 3/5。progress 只作展示，不放行阶段、不关闭 finding、不推导 `done`。
