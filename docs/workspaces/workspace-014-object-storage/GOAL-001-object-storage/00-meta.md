...
| 阶段 | 内容 | 先后 | 状态 |
|------|------|------|------|
| R1 | **端口与配置面冻结**：公共类型无本地路径 / `os.File`；命名空间隔离三类落盘；缺省本地盘；S3 为显式配置；裁定 List/GC 是否进端口（I-005）与桶模型（I-002） | 起点 | 未开始 |
| R2 | **S3 兼容接入**：驱动公约数（I-001）、凭证键名（I-003）、配置后 `readyz` 扩依赖 | 依赖 R1 | 未开始 |
...
`progress` = 已完成阶段数 / 5。当前 `0/5`。progress 不放行、不关门。
...
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| I-001 | required | S3 API 子集与驱动：MinIO / R2 / AWS 的最低公约数；禁止第三对象存储方言 | R2 方案冻结 / 实施 | R2 实施前 | R2 决策 + 编译/接入证据 | open | — | 对应 VP I-014-001 |
| I-002 | required | 桶模型：单桶 + 前缀 vs 多桶；三类落盘的 key 隔离规则 | R1 方案冻结 | R1 端口冻结 | R1 决策 | open | — | 对应 VP I-014-002 |
| I-003 | required | 配置键名与凭证注入（YAML + env fail-closed；secret 不入库） | R2 方案冻结 | R2 实施前 | R2 决策 | open | — | 对应 VP I-014-003 |
| I-004 | non-blocking | 存量本地文件如何进入对象存储？ | R5 关门叙事 | R5 | 点名 residual | **recorded** | 用户已裁决不进退出分母 | 不提供产品搬运器；既有存量 = 继续本地或运维自备拷贝（对应 VP I-014-004） |
| I-005 | required | 启动 GC / 列举是否属于端口（List），还是本地适配器私有 + DB 引用集？ | R1 方案冻结 | R1 端口冻结 | R1 决策 | open | — | 现行 `RasterAssetStore` 目录 GC；S3 无本地列举 |
