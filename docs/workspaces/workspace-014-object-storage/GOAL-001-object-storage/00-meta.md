...
| 阶段 | 内容 | 先后 | 状态 |
|------|------|------|------|
| R1 | **端口与配置面冻结**：公共类型无本地路径 / `os.File`；命名空间隔离三类落盘；缺省本地盘；S3 为显式配置；裁定 List/GC 是否进端口（I-005）与桶模型（I-002）。承载子目标：GOAL-002-object-port-freeze | 起点 | 进行中（D-002/D-003 已裁，方案冻结见 GOAL-002 D-001） |
