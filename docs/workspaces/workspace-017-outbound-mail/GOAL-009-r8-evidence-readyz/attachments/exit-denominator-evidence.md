...
| **live 已实跑（2026-08-24，用户裁决补跑；域名验证后复跑通过）** | `internal/mail/resend_live_test.go` 两轮实跑：①首试 from=eshowy.top 被 403 拒（域名未验证）；②用户在 resend.com/domains 完成验证后**以正式发件地址复跑 PASS**——Ping 2xx + POST /emails 2xx（from=admin@eshowy.top → magicvr@hotmail.com），另 resend.dev 沙箱路径亦曾验证成功。生产配置与测试配置完全一致 |
