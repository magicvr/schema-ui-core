先加载 `/audit` 技能，再按独立审计规则核对 workspace-017 的 Root 关门材料。技能要求只出意见、不改状态。本轮按你的约束不落盘，先只读核对工作区、Root 五件套、子目标和愿景材料。工作区绑定已核对。接着通读 Root 与四个子目标的五件套，并对照代码事实。接下来核对四个子目标的合同、执行证据和自审意见，再对照代码。文档层已读完。接下来独立核对代码合同、接线、sweep 与测试证据。接着核对 readyz 探针接线、仓库 sweep，并独立跑本波相关测试与 `store` 包 diff。## A-002 · Root 关门独立审计（R1～R4 全阶段 + 方向级退出判据）
...
| VP-017 判据 5 · 非目标未越界 | **成立**。`git diff --name-only be23164..HEAD -- apps/api` 仅 19 个 mail/config/composition/README/configs 文件；无 users email 列、邀请、自助恢复、模板、SMS、第二拨号、Charter 改写 | 本轮 `git diff --stat be23164..HEAD -- apps/api` |
...
| `internal/store` 两测失败 | **不阻塞本次关门**。本轮确认 `git diff be23164..HEAD -- apps/api/internal/store` 为空；两测为 `PG_TEST_*` 门控 live-Postgres（`postgres_test.go`），属共享 probe 库环境/遗留库状态，不在本区 diff 与退出分母内 | `apps/api/internal/store/postgres_test.go:461-547`；本轮 git diff |
...
本意见不修改 status/progress；请用 `/govern` 响应本意见后将 Root 标为 `done`。
