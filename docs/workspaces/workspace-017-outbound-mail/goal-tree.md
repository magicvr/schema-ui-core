...
历史 Root 关门（A-001 self pass + A-002 independent pass）原文保留，**已由用户否决其作为现行 `done` 的效力**。开放 required Goal finding = 0。R7～R8 子目标按 P-001 尚未创建；下一阶段 = R7（设置/热切换/试发），开设前须关闭 I-009（用户裁决点）。GOAL-007 A-001 F-001 residual（outbox 读面复用 settings.read 门禁）在 R7 开设时复核。
