...
- `permissionCascade`、`permissionIntent` 与 `permissions.inheritance` 不是 v2.2 的合法协议字段或 capability；
- 本文进入 v2.3 `protocol-manifest.json` 的 `authority.semanticSpecs`；
- Renderer 只有在页面同时声明 `meta.protocolVersion: "2.3"` 与 `permissions.inheritance` 后才可启用本 ADR 的语义；不得为旧页面追溯开启继承。
...
> 来源：审计 0065（协议制品外过程记录，`0065/V287`–`V294`）。下表是**提案层裁决**，关闭的是可接受性歧义，不代表用户已接受 ADR，也不提前授权 Schema/L2/fixtures 落地。
...
- **2026-07-23：** 用户确认 `OQ-23-1` 的范围裁决：`table.props.columns[]` 不参与权限级联，仅 `table.props.actions[]` / `table.props.toolbar[]` 作为表格操作挂载边参与。列继续仅适用本地既有 `permissions`。
- 此确认发生在接受前，锁定了提案边界；其后用户已单独接受本 ADR 并授权按新的 MINOR 执行 M1–M6。

## 审计结论（0065 响应，2026-07-23）

- V287–V294 已写回本 ADR 的裁决表；裁决经用户接受后提升为 v2.3 权威语义，而非回写到 v2.2。
- 0065 的“不可直接 accepted”条件已通过文档裁决和用户接受决议消除；本 ADR 可进入 `protocol-manifest.json`。
- M1–M6 必须作为同一 v2.3 变更集完成，并由发布门禁验证 Schema、L2/L3a、fixtures、Renderer 规范和制品。
- 细节与原始问题证据保留在协议制品外的 `docs/audit/archived/0065-2026-07-23-review.md`。
