...
- **Root** `GOAL-001-rate-limiter-atomic-port`：**`active`** · **0/3**（R1 合同落盘 → R2 内存实现+迁移+并发回归 → R3 证据与关门），纲领见 Root `00-meta.md`。
...
| Root Goal | `GOAL-001-rate-limiter-atomic-port` | `parent: null`；**active** · 0/3 |
...
| 阶段 | 内容 | 状态 |
|------|------|------|
| R1 | **合同落盘**（GOAL-002）：D-002 冻结 + kernel.AllowRecord + Memory 单锁实现 + 合同级测试 | **C1/C2 已关门** · C3 self 待做 |
| R2 | **14 处迁移 + handler 回归**（判据 2/3）：按立即消费 / 失败预算两口径迁生产调用点 | 待 R1 关门 |
| R3 | **证据与关门**（判据 4/5）：证据矩阵 / 越界核账 / 审计闭合 | 待 R1–R2 |
