...
| R4 | **证据与关门**：快测 + `zh-CN`/`en-US` 双 locale 范例；无越界（汇率/计费/RT-T03）；required = 0 | 依赖 R3 | **已关门**（GOAL-005 done 4/4 · Root 关门审计 A-001 self + A-002 grok independent 双 pass；用户 2026-08-27 书面确认） |

`progress` = 已完成阶段数 / 4。当前 **4/4**（2026-08-27：R1～R4 全部关门，Root done）。
...
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| I-001 | required | 时区来源：会话级（客户端 / 请求探测）vs 用户级（存库）vs 两者；影响设置归属与 schema | 方案冻结 | R1 | 用户裁决 | **verified** | — | 2026-08-26 用户裁决：会话级 auto + 站点兜底 + 用户级 localStorage 覆盖（D-002 accepted） |
| I-002 | required | 数字 / 货币语义落点：仅前端展示 vs 后端 API 序列化也携带语义（如 decimal 字符串合同） | 方案冻结 | R1 | 用户裁决 | **verified** | — | 2026-08-26 用户裁决：前端落点；API 保持机器合同并文档化（D-002 accepted） |
...
| I-005 | non-blocking | 默认时区 / 数字 / 货币的配置归属：Settings 哪一 tab、哪些字段 | 方案冻结 | R2 | lead 提案 + 用户确认 | **verified** | — | 2026-08-26 用户确认：站点默认进 Localization tab（新增 defaultCurrency）；用户级覆盖并入头部 locale 通道（D-002 accepted） |
