...
| R3 | **数字 / 货币语义**：locale 驱动千分位 / 小数位 / 百分比 / ISO 4217 展示与输入解析合同 | 依赖 R2 | **已立项**（GOAL-004 **active** · 方案 D-001 冻结；审计模式 independent（migration 涉及）） |
| R4 | **证据与关门**：快测 + `zh-CN`/`en-US` 双 locale 范例；无越界（汇率/计费/RT-T03）；required = 0 | 依赖 R3 | **待立项**（GOAL-005） |

`progress` = 已完成阶段数 / 4。当前 **2/4**（R1/R2 已关门；R3 已立项未完成；R4 待立项）。
