...
- [ ] 判据 #1（Webhook 合同）：secret 校验 fail-closed；无/错 secret 不可被当成合法 Update；有测试——R2（R1 冻结无 token 策略）
- [ ] 判据 #2（分发端口）：命令与 callback 的 Register/Unregister + 分发有测试；未知命令有确定回落（不 panic）——R2
- [ ] 判据 #3（出站端口）：`SendMessage` 文本可测（mock 供应商）；生产供应商不把 Bot 客户端类型漏进模块公共契约——R3（R1 冻结 HTTP vs SDK）
- [ ] 判据 #4（身份映射）：同一 `telegram_user_id` 多次 get-or-create 得到同一 `subject_id`；不写 `admin.users`——R2
- [ ] 判据 #5（设置与密钥）：Admin 可配置 token/secret；密钥 fail-closed；不进配置包明文——R3
- [x] 判据 #6（限流评估落盘）：激活前书面评估 VP-027 进程内 limiter 对 webhook/`chat_id`/`telegram_user_id` 是否足够——**已核销**（2026-09-03 · VRev-070 §6：进程内够用、不需要 Redis）。桶分母与 Record/Clear 映射仍属 R1（I-030-003/006）
