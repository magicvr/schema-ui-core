...
- [x] 判据 #5（设置与密钥）：Admin 可配置 token/secret；密钥 fail-closed；不进配置包明文——已由 GOAL-004 完成
...
| R4 | 证据与关门（判据 7/8；依赖 R1–R3）：证据矩阵 / 越界核账 / 审计闭合 | **进行中**（待立项 GOAL-005） |
...
| I-030-005 | non-blocking | 设置是否热切换 token（mail 有热切换先例）还是重启生效。 | 退出判据 5 | R3 | 用户裁决（GOAL-004 D-001） | **verified** | — | 2026-09-03 用户裁决：热切换（沿用 Mail 先例），D-001 |
