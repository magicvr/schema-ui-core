...
└── GOAL-007-list-page-visual-alignment [active · 6/7]
```
...
R1 分母与语义冻结 [done · GOAL-002 · 3/3]
  ...
  → R6 列表页视觉与筛选体验收敛 [active · GOAL-007 · 6/7]
```
...
|----|-------|--------|----------|--------|-------|
| GOAL-001-admin-workflow-continuity | Admin 工作流连续性与安全反馈交付 | **active** | 4/6 | null | R1/R2/R3/R4 done；R6 `GOAL-007` 已完成原 C1～C4 后按用户选择 A 回开，现为 active 6/7（追加 C5 布局修订与 C7 控制位纠偏）；I-037-001～004/007 verified；R1/R2/R3/R4/R6 历史审计已记录，R6 C6 修订审计待完成；I-037-006 verified；Vision open required = 0；R5 `GOAL-006` active 3/4；R5-I-004 用户书面关门确认仍开放 |
...
| GOAL-007-list-page-visual-alignment | R6 列表页视觉与筛选体验收敛 | **active** | 6/7 | GOAL-001-admin-workflow-continuity | 原 C1～C4 与 A-001 self 已完成；按 D-012 回开，C5 实现/回归（E-004）与 C7 控制位纠偏（D-003/E-005）已完成，C6 修订审计待完成；不改变顶部功能栏、左侧导航与查询/重置合同 |

## 维护说明

- Root `progress: 4/6` 由 `00-meta.md` 的 6 个显式纲领检查点派生；R6 回开与追加 C7 不改变六阶段分母（C7 只改变 R6 子目标自身的 7 项检查点）。它只表示阶段检查点展示，不放行方案、实施、验收或关门。R5 用户确认门禁仍开放，因此 Root/VP 尚未完成。
