...
- 状态：**in-scope 部分 fixed / cross-workspace 部分已按用户裁决移交**
  - **fixed（本目标范围内）**：E-004/E-005/E-006 的证据表述已更正为 `tsc -b` 并注明原口径空转；本轮及后续 R6 验证一律使用 `tsc -b`。该更正属对**自身失实证据的纠错**，非取舍决策，故无需用户裁决即可闭合。
  - **已裁决并移交（跨工作区）**：用户于 2026-09-18 按 P-004 裁决处置路径为**方案 A（立独立目标系统性处置）**，据此开设 `GOAL-008-typecheck-evidence-convention`（`active · 2/4`，非纲领阶段）承接跨工作区部分，见 Root `D-013`/`E-020`。workspace-002/009/010/011 的同类条目在该目标内登记处置，不在本目标 canonical 范围。
...
C5/C7/C8 的实现与回归经独立复核**属实**，C8 的 token 边界修订合规且已进入生产构建产物。本轮两项 required 均已按合法路径处置：

- **F-001**（决策层投影落后）：**fixed**，VP-037 与 `docs/vision/workspaces.md` 已同步至 `active · 7/8` 并补记 C7/C8 规划短史。
- **F-005**（`tsc --noEmit` 空转，high）：**in-scope fixed**（E-004/E-005/E-006 证据已更正为 `tsc -b`）；**cross-workspace 部分已按用户 P-004 裁决（方案 A）移交 `GOAL-008-typecheck-evidence-convention`**（Root `D-013`/`E-020`）。

F-002、F-004 已 `fixed`；F-003（列表视觉面缺持久化浏览器级回归）为 recommended 保持 open，建议作为 R6 后续或独立小目标处理。

**verdict 维持 `conditional`** 的理由：F-005 的跨工作区部分虽已合法移交，但其闭环（`GOAL-008` 的 C3/C4）尚未完成。按 P-003，`fixed` 需可核对修正、移交不等于闭合，故本审计不将 F-005 记为已闭合。

**R6 完成投影**：按用户 2026-09-18 裁决「等 F-005 裁决落定后再关门」，R6 保持 `active · 7/8`；待 `GOAL-008` 处置落定后单独执行 `done · 8/8` 与 Root `active · 5/6` 投影。R5-I-004 用户书面关门确认仍开放，本审计不关闭 Root/VP。
