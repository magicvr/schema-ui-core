...
- R6 子目标 `GOAL-007-list-page-visual-alignment` 原已完成 `done · 4/4`；用户选择 A 后回开为 **`active · 7/8`**，C5 布局修订、C7 控制位纠偏（图标强调、搜索配对回调、视图表单归属）与 C8（页面 actions 高度统一、`--control` 折叠开关语义 token、空展开抑制）均已完成，C6 修订审计待完成；不改变 shell、查询/重置逻辑或 Saved View 存储格式。
...
| R6 | 参考范例页收敛通用列表、筛选折叠、视图/页面级按钮布局与单页分页 | **active via `GOAL-007-list-page-visual-alignment` · 7/8**；C5 布局修订、C7 控制位纠偏与 C8 控件语义已完成，C6 修订审计待完成；不改变顶部栏、左侧导航与查询/重置合同 |
