...
- 保存范围是当前表格查询与列配置 allowlist：`q`、`filters`、`sort`、`order`、`pageSize` 与列可见性。
- 不保存当前页、选择集、record view 或 modal 草稿；恢复从第一页开始。
- 只保证同一浏览器、同一设备上的刷新后恢复；不提供跨设备同步、跨用户共享、最近/收藏或协作权限。
- 存储键的各段按稳定编码处理；读取到 malformed、未知字段、超出 allowlist 或当前权限/Schema 边界的视图时 fail closed，不污染当前页面状态。
- 浏览器存储不可用、读写异常或容量不足时，必须通过统一反馈呈现错误；不得伪装成已持久化。
- 本决策不新增 API、数据库表、迁移或服务端持久化；实际序列化、校验与失效测试完成前，I-037-002 仍为 `collecting`，不得提前标记 `verified`。
