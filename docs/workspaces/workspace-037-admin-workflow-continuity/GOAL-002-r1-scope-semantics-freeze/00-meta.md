...
- [ ] C2：Saved View、dirty-state、Toast/API 错误/恢复语义的取舍形成已确认决策。
- [ ] C3：I-037-001～004 关闭，R1 self 审计通过，并将 Root R1 检查点投影为完成。
...
- `GOAL-001-admin-workflow-continuity`（R1 阶段子目标；父目标保持 `active · 0/5`，直到本目标完成并通过 R1 审计）。
