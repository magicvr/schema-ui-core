...
| I-002 | required | Tracing：OTLP HTTP vs gRPC、采样默认、未配置 endpoint 的 no-op | R3 方案冻结 / 实施 | R3 接入前 | R3 决策 | verified（2026-08-22：OTLP/HTTP；ParentBased+ratio 缺省 1.0；disabled 纯 no-op） | — | GOAL-004 D-001（`GOAL-004-otel-traces/01-decision/D-001-tracing-contract.md`）；对应 VP I-015-002 |
...
| I-005 | required | request-id / correlation 如何写入 span（属性名、是否 baggage） | R4 方案冻结 | R4 关联前 | R4 决策 | verified（2026-08-22：属性 `correlation.request_id` + baggage 键 `request-id`；判据测试锁定） | — | GOAL-005 D-001（`GOAL-005-requestid-correlation/01-decision/D-001-requestid-correlation.md`）；对应 VP I-015-005 |
