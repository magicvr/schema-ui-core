...
```text
GOAL-001-config-export-diff-dryrun-import（active · 1/4 · primary_plan = VP-025-config-export-diff-dryrun-import）
├── GOAL-002-r1-contract-freeze（done · 3/3 · R1 合同冻结：配置包合同 v0.1.0）
└── 纲领阶段（子目标待立项）：
    R2 导出+diff（GOAL-003 候选）→ R3 dry-run+导入（GOAL-004 候选）→ R4 证据与关门（GOAL-005 候选）
```
...
|----|-------|--------|----------|--------|
| GOAL-001-config-export-diff-dryrun-import | 配置包导出 / diff / dry-run / 导入 | active | 1/4 | null |
