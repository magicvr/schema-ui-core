...
| 编号 | 问题/所需信息 | 级别 | 影响的门禁 | 最晚需要阶段 | 验证/收集动作 | 状态 | 备注 |
|------|--------------|------|-----------|--------------|----------------|------|------|
| I-001 | 生产环境 TLS 终结拓扑（HSTS 在哪一跳生效、nginx 是否处于 TLS 之后） | non-blocking | F-020 HSTS 生效性宣称 | 关门（不阻断——HSTS 头已按 RFC 6797 无害实施，HTTP 上被浏览器忽略、TLS 接入自动生效） | 运维侧确认部署拓扑后回填 | deferred（复审触发：生产 TLS 终结方案确定时） | 来源 D-001/F-020；A-003 R-F003 登记要求 |

> 无其他开放 required 信息项；本表由 A-003 R-F003 补登。
