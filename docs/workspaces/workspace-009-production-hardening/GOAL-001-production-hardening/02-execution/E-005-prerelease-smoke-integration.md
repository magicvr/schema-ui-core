...
- CI：`.github/workflows/r6-basic-matrix.yml` container-smoke 改为直接调用 `scripts/pre-release-smoke.sh`（隔离栈 + API loopback override + SMOKE_CSP + C-006 persistence + 自动清理），不再手工重复 build/start/wait/smoke/persistence/teardown；W16-F01 改密由 smoke SM-004 真实完成。
