...
title: 分发形态正式化工作区（cli+包 对外服务化收口）
status: active
...
本工作区是 [VP-024-distribution-formalization](../../vision/plans/VP-024-distribution-formalization.md)（**`active`** v0.2.0 · 2026-08-29 激活）的唯一 lead delivery workspace。**组合层平台波**：收口 VP-022/023 go 后合并残余 7 项，把「cli+包 分发路径」升级为对外正式化（serve 壳 / npmjs 公开发布 / compose CI 实跑 / fork 对照计时 / 六包形态细化 / 迁移工具化 / 方法 B 置顶）；不改 Charter、不弃 fork。

- **Root** `GOAL-001-distribution-formalization`：**`active`** · 0/7（R1 serve 壳 → R2 公开发布 → R3 compose/CI 实跑 → R4 fork 对照计时 → R5 六包形态细化 → R6 迁移工具化 → R7 置顶与收口报告）。
...
| Root Goal | `GOAL-001-distribution-formalization` | `parent: null`；**active** · 0/7（2026-08-29 开区） |
...
| 规划对齐 | `primary_plan` = `VP-024-distribution-formalization`（`active` v0.2.0） | 2026-08-29 激活/开区（VRev-052 self `pass`；架构类轻量 freshness PASS） |
...
| 阶段 | 内容 | 状态 |
|------|------|------|
| R1 | **serve 壳闭环**（判据 #1）：`schema-ui serve` 子命令（HTTP 壳 + config 装载 + assembly 服务器面）；create 骨架直接 `serve` 启动；RT-D02 合同接线 | 未开（开区 0/7） |
| R2 | **公开发布通道**（判据 #2）：npmjs.com 六包 + CLI 公开发布；golden-field 免凭据消费实证；发布流程成文（**R2 前置门禁 I-024-001：用户授权正式 scope/凭据**） | 依赖 R1（可评估并行） |
| R3 | **compose/CI 实跑**（判据 #3）：compose/Dockerfile + consumer-regression workflow 真实 CI（或用户环境等价）实跑 PASS；workflow 补 pnpm setup 与 token 注记 | 依赖 R1/R2 |
| R4 | **fork 对照计时**（判据 #4）：同一演进集 fork 同步 vs 包路径 bump 实测对比（耗时/冲突/迁移成本）定量结论 | 依赖 R2/R3 |
| R5 | **六包形态细化**（判据 #5/#6）：renderer 依赖图 external 化（ui 包可消费）；纯原子拆分；冻结面升格 v1.4.0 | 依赖 R1 |
| R6 | **迁移工具化**（判据 #7）：fork→包迁移指南配套工具（`schema-ui migrate-fork` 或等价） | 依赖 R2 |
| R7 | **置顶与收口报告**（判据 #8）：QUICKSTART 方法 B 置顶（fork 第二路径·Charter 不动）+ 收口报告（核销表/公开消费往返实证/fork 对照结论/残余清零） | 依赖 R1–R6 |
