...
- **Root** `GOAL-001-distribution-formalization`：**`active`** · 6/7（R1 serve 壳 → R2 公开发布 → R3 compose/CI 实跑 → R4 fork 对照计时 → R5 六包形态细化 → R6 迁移工具化 → R7 置顶与收口报告）。
...
| R7 | **置顶与收口报告**（判据 #8）：QUICKSTART 方法 B 置顶（fork 第二路径·Charter 不动）+ 收口报告（核销表/公开消费往返实证/fork 对照结论/残余清零） | **进行中**（GOAL-008 active · Root 关门审计 pass —— 待用户书面确认） |
