...
承接 Root R2 与 VP-024 判据 #2：把 R1 的 serve 面与六包**真实公开发布**——npmjs.com `@magicvr/schema-ui-*` 六包（**scope 定稿 @magicvr**，2026-08-29 用户裁决：维持现行发布形式，不再改用 `@schema-ui` 形态）、Go `apps/api/v0.4.0` origin tag + 公共 proxy；golden-field 升级到 v0.4.0 + 六包并完成**无凭据消费实证**（npmjs 公开免 token · go proxy 免认证）；发布流程成文（脚本 + 凭据注入点 + scope 迁移 changelog 注记）。核销 go 后清单第 ② 项（npmjs 公开可见性）与 R1 残余 2（registry 级骨架消费）的发布前提。

## 成功标准（可验证检查点）

- [x] C1：npmjs.com 真实发布六包 `@magicvr/schema-ui-{protocol,lib,theme,ui,renderer,shell}`（lib/theme/ui/shell 0.1.0 · protocol/renderer 0.2.0）——scope 定稿 @magicvr（用户裁决 2026-08-29 · D-001 §7）
...
| S4 | 证据 + 自审关门（含 R1 残余 2 核销前提确认） | **已关门**（2026-08-29 · A-002 grok independent fail → F-001~F-004 fixed · 用户确认 · scope 定稿 @magicvr） |

## 信息就绪与未知项（P-005）

| ID | 级别 | 所需信息 / 问题 | 影响门禁 | 最晚需要阶段 | 验证 / 收集动作 | 状态 | 延期 / 复核 | 证据 / 结论 |
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| I-024-001 | required | npmjs 公开发布授权（scope 名 + 凭据方式） | 判据 #2 | R2 | 用户书面授权或裁决 | **verified**（2026-08-29 用户裁决：@magicvr 定稿真实发布 · token = 仓库根 `.env` `npm_token`） | — | 用户裁决（goal round）· D-001 §7 |
