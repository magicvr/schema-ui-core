...
| 阶段 | 内容 | 先后 | 状态 |
|------|------|------|------|
| R1 | **端口与配置面冻结**：Tx 公共类型 ≠ `*sql.Tx`；方言由配置选择；缺省 `db.path` SQLite；PG DSN 键名冻结；v1.3 补时间宽度（sqlite INTEGER / postgres BIGINT）与 R2 Open+Ping 证据边界 | 起点 | ✅ GOAL-002（D-001 / A-001；D-002 / A-003 闭合 A-002；D-003 / A-005 闭合 A-004；**D-004 / A-007** 闭合 A-006 required；合同 **v1.3**） |
