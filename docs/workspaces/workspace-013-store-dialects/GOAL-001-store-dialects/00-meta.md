...
| 阶段 | 内容 | 先后 | 状态 |
|------|------|------|------|
| R1 | **端口与配置面冻结**：Tx 公共类型 ≠ `*sql.Tx`；方言由配置选择；缺省 `db.path` SQLite；PG DSN 键名冻结 | 起点 | 未开始 |
...
| I-003 | non-blocking | 哪些模块公共 API / 内核类型泄漏 `*sql.Tx` | R4 范围 | R4 方案 | 代码扫描清单 | **open** | R1 扫描即可收集 | 待确认 |
