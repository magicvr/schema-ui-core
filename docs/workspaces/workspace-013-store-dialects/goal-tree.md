...
> Root：`GOAL-001-store-dialects`（**done**；R1–R5 5/5）
> primary_plan：`VP-013-store-dialects`（**active** · 架构 A1）

## 树

```text
GOAL-001-store-dialects [done]      · Store 双方言（PostgreSQL 生产权威 + SQLite 内嵌）
...
├── GOAL-005-r4-repository-surface [done]   · R4 仓库公共面收口（*sql.Tx → kernel.Store/Tx + postgres 启动）
└── GOAL-006-r5-dual-path-acceptance [done] · R5 双路径验收（升级策略 + 备份合同 + 共事务 + Root 关门）
```
...
|----|-------|--------|--------|----------|---------|
| GOAL-001-store-dialects | Store 双方言（PostgreSQL 生产权威 + SQLite 内嵌） | null | done | 5/5 | 2026-08-20 |
...
| GOAL-005-r4-repository-surface | R4 · 仓库公共面收口（`*sql.Tx` → kernel.Store/Tx + postgres 启动） | GOAL-001-store-dialects | done | 6/6 | 2026-08-20 |
| GOAL-006-r5-dual-path-acceptance | R5 · 双路径验收（升级策略 + 备份合同 + 共事务） | GOAL-001-store-dialects | done | 5/5 | 2026-08-20 |
