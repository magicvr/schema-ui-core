...
| 阶段 | 内容 | 状态 |
|------|------|------|
| U0 | 双路径回归基线：sqlite 全量 `go test ./...` + live PG 全量 boot + 完整启动（既有证据固化） | 待做 |
| U1 | **I-001 升级策略**（SQLite→PG）：结论/残余（in-place vs dump/restore vs fresh bootstrap），书面落盘 + 抽样证据 | 待做 |
| U2 | **I-004 备份/恢复合同**：替代 `VACUUM INTO` 的 PG 生产路径（pg_dump 或等价），落盘 + 可执行验证 | 待做 |
| U3 | 跨模块共事务验收（job/审计/钱包等共用事务在 PG 上的一致性与回滚）+ `readyz` 生产向就绪 | 待做 |
| U4 | 关门：VP-013 退出判据 1–6 全链核对 + self + independent（production 门禁）→ GOAL-006 done → **Root 5/5 关门** | 待做 |
...
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| RT-I-001 | required | 存量 SQLite→PG：in-place vs dump/restore vs fresh bootstrap | U1 / Exit 2 | U1 | 原型/抽样 | open | 责任人：本区编排 | 待确认 |
| RT-I-004 | required | PG 备份/恢复合同（替代 `VACUUM INTO`）samping | U2 / Exit 4 | U2 | 设计 + 验证 | open | 责任人：本区编排 | 待确认 |
