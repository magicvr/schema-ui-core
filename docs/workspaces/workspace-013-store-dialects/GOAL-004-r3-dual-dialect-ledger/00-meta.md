...
| T4 | 证据与关门：sqlite 回归 + PG fresh bootstrap 全量 apply；self + independent（迁移/数据门禁） | ✅ 2026-08-20（sqlite 全量回归 0 FAIL；live PG 全量 boot + 台账幂等 + 系统级无 int 时间检查；self A-001~A-004/A-006 + independent A-005 → **done**） |
...
| I-002 | required | 各迁移时间列单位、非时间 INTEGER 宽度、布尔列的**逐表/逐列**证据 | T3 对写 DDL | T3 每迁移前 | 代码/数据核对 | **verified** | 2026-08-20（A-005 F-001 闭合） | 时间列（秒/毫秒按列沿用）PG 一律 `BIGINT`（48 迁移 + 系统级「无 int 时间列」检查 0 残留）；wallet 金额列（balance_* / amount_delta / balance_after_*）`BIGINT`；布尔列两方言 `INTEGER` 0/1（Go 写 0/1 整数）；非时间计数（progress/attempt/fail_count/version/sort 等）`INTEGER`（int4 内） |
| I-003 | required | catalog 按方言分列 vs 单一 catalog 成对 SQL 的选择（checksum 绑定影响） | T0 方案 | T0 前 | R1 v1.4 §4 两种合法解取舍 | **closed** | 2026-08-20（A-005/A-006） | 采纳 v1.4 §4 方案 1：**同一 compiled catalog + 每迁移 PostgresApply 对写**，checksum 仍绑 sqlite 历史、PG 对写另留证据 |
| I-004 | non-blocking | PG fresh bootstrap 的时间/备份语义是否与 sqlite 完全一致 | T4 证据 | T4 | live PG 对比 | **closed** | 2026-08-20 | 全量 PG fresh boot + WasFresh 翻转 + 台账幂等已验证；PG 备份合同属 R5（Root I-004） |
