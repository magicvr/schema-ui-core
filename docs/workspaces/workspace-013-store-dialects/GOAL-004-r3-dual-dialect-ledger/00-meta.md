...
| T3 | 逐迁移对写：authsession `sqlite_master`/`PRAGMA`/`COLLATE NOCASE`、时间 `BIGINT`、非时间 INTEGER 宽度、布尔、可 rebind 文本；双 apply + checksum | 🔄 E-004/A-003：12/13 模块已对写 + 全量 PG boot + BIGINT 断言；**operationlog 待续 + 生产解闸** |
...
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| I-001 | required | compiled 台账当前逐迁移 SQL 方言债的**完整**清单（不是 R1 抽样） | T3 对写 | T3 前 | 迁移包代码扫描 | **verified（12/13）** | 2026-08-20 E-004 | 12 模块已对写（BIGINT 时间/金额、CITEXT、partial index）；**operationlog 为剩余项**（T3 收尾） |
