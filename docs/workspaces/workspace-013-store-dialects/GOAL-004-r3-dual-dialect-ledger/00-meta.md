...
| 阶段 | 内容 | 状态 |
|------|------|------|
| T0 | R3 方案冻结：catalog 分列/成对形态、checksum 绑定、Apply→`kernel.Tx`、`LIKE`/`COLLATE`/时间/宽度/布尔落盘规则 | 本目标 D-001 |
| T1 | `kernel.MigrationContribution.Apply/Reconcile` → `func(kernel.Tx)`；store 迁移运行器改走 `Run`（sqlite 保持绿） | 待做 |
| T2 | store postgres 迁移运行器：fresh bootstrap / apply postgres catalog / `schema_migrations` 台账（checksum 同 sqlite 绑定） | 待做 |
| T3 | 逐迁移对写：authsession `sqlite_master`/`PRAGMA`/`COLLATE NOCASE`、时间 `BIGINT`、非时间 INTEGER 宽度、布尔、可 rebind 文本；双 apply + checksum | 待做 |
...
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| I-001 | required | compiled 台账当前逐迁移 SQL 方言债的**完整**清单（不是 R1 抽样） | T0 方案 / T3 对写 | T0 前 | 迁移包代码扫描 | collecting | 责任人：本区编排 | R2 期已扫：authsession migration（sqlite_master/PRAGMA/COLLATE NOCASE）、时间/宽度/布尔按列查证；**待补全**：全部迁移逐条核对 |
