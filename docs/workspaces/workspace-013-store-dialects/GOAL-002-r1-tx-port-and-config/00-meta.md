...
serves_summary: 冻结内核持久化端口（Tx ≠ *sql.Tx）与 db 配置键名；不实现驱动、不对写台账。
---
...
权威正文：[attachments/r1-tx-port-and-config-freeze.md](attachments/r1-tx-port-and-config-freeze.md)（D-001）。
...
| S1 | 冻结端口 + 配置键；self 审视 | ✅ D-001 / A-001 |
...
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| I-001 | required | 现行打开路径与 WithTx 形状是否足以冻结端口 | S1 冻结 | S1 前 | 读 store/config/composition | **verified** | 2026-08-20 | E-001：`OpenWithCatalog(cfg.DBPath)` + `WithTx(*sql.Tx)` |
| I-002 | required | 本目标审计模式 | S1 关门 | S1 前 | 按内核契约、无代码 | **verified** | 2026-08-20 | D-001：`self`；independent 延至 R2 代码 |
