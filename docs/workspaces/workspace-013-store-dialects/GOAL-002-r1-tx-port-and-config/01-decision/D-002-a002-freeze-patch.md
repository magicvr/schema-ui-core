...
R2 立项前先冻结 Root I-002（驱动）。实施合同已被 [D-003](D-003-a004-freeze-patch.md) 升为 **v1.2**（本条 v1.1 的 upsert / path / `WasFresh` 骨架仍成立；时间「一律秒」与 `Open(..., catalog)` 时序以 v1.2 为准）。建议 R2 方案冻结后再 `/audit`（independent）。
