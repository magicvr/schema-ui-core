...
- [ ] **R4 结果中心与体验收敛**：列表/详情/进度/终态/过期/重试/取消/下载；中英文、浅色深色、加载空态错误态、可访问。→ 由 `GOAL-005-r4-result-center-experience` 承接（**`active · 3/4`**；C1～C3 已完成，C4 审计与投影 pending）。
...
- workspace/Root scaffold 是已发生事实；纲领 R1、R2、R3 已由 `GOAL-002` / `GOAL-003` / `GOAL-004` 交付并关门（各 `done · 4/4`），R4 由 `GOAL-005` 承接（`active · 3/4`）；`progress: 3/5` 只由上方 5 个显式检查点派生，不放行阶段、不关闭 finding、不覆盖 status。
