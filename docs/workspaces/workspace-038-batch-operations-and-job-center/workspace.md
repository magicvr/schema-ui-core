...
- Root `[workspace-038-batch-operations-and-job-center] GOAL-001-batch-operations-and-job-center`：**`active · 0/5`**；纲领 R1→R5。
- 用户已确认 workspace slug = `workspace-038-batch-operations-and-job-center`；Root slug = `GOAL-001-batch-operations-and-job-center`。
- Vision open required：0；`V-F125` → `fixed`；`V-F126` 保持 `open · recommended`，由 `I-038-003`（R1 前 required）承接。
- R1 前须关闭 `I-038-001`～`003`（Job 分母与作用域 / 批量异步契约与协议面 / 首波批量操作分母）。
...
| Root Goal | `GOAL-001-batch-operations-and-job-center` | `parent: null`；**active · 0/5** |
...
- VP：[VP-038-batch-operations-and-job-center](../../vision/plans/VP-038-batch-operations-and-job-center.md)（**`active` · v0.2.0**）
...
- Vision open required：0；`V-F126` `open · recommended`（由 `I-038-003` 承接）；`I-038-004`/`I-038-005` `verified`；`I-038-001`～`003` `open`（R1 前 required）；`I-038-006` `deferred · non-blocking`

## 纲领阶段

| 阶段 | 目的 | 状态 |
|------|------|------|
| R1 | 分母与契约冻结：Job 种类×作用域矩阵、批量异步契约（含同步 `batch-delete` 兼容口径）、首波批量操作分母、权限/Profile 边界与排除项 | **pending**；`I-038-001`～`003` 为进入门禁 |
| R2 | 通用作业读面：Job 列表/详情/结果读取 API + 权限与作用域过滤 + fail-closed；不改变 Job 六态合同 | **pending** |
