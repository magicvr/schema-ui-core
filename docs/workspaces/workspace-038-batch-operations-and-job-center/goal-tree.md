...
- **R2 子目标 `GOAL-003` 于 2026-09-19 立项**（`active · 0/4`）：C1 模块与权限接线 / C2 查询与索引 / C3 读面 API 与作用域 / C4 R2 审计与投影；审计模式 `cross`。承接 R1 `D-001` §5 的 T-1～T-6 与未定项 O-1～O-3（其中 O-3 = `I-038-007` required）。
