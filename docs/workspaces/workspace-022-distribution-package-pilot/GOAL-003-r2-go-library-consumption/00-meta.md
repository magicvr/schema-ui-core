...
- [ ] **S1 · 阻断验证与外移方案**：internal 规则实验证据（E-001）；外移方案评估与**用户裁决**（D-001：目录提升 / 独立模块 / 其他）
- [ ] **S2 · 外移重构**：A 层（kernel）+ B 层（modules）移出 `internal/` 并按用户裁决落地；全仓 import 改写；构建/测试回归全绿（冻结面清单 §1/§2 路径同步 v1.0.1）
- [ ] **S3 · 黄金下游仓装配闭环**：`attachments/golden-consumer`（嵌套 go.mod + replace）自建组合根装配 kernel + ≥1 标准模块；迁移台账收集、启动冒烟、双方言 apply 与 fork 形态功能基线等价；**F-001（C 层泄漏）/F-002（B 层符号回填）fixed**
