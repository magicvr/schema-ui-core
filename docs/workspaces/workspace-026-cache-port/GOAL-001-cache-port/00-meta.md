...
- [ ] 判据 #1（端口契约冻结）：Cache 端口（Get/Set/Delete + TTL + 命名空间 + 并发安全）供应商无关、快测可断言——R1
- [ ] 判据 #2（双策略 + 可插拔）：绝对过期 + 滑动过期 + 策略接口（含自定义策略测试样例）——R2
- [ ] 判据 #3（内存供应商可用）：有界容量 + TTL 清理 + 驱逐 + 并发边界测试——R2
...
- [ ] 判据 #6（停机语义）：后台清理协程（若选）声明 SIGTERM 排空；否则惰性清理——R1/R2
...
| 阶段 | 内容 | 检查点/状态 |
|------|------|-------------|
| R1 | 合同冻结（判据 #1/#6 边界）：Cache 端口 API 形态（I-026-001）· TTL/清理语义（I-026-002）· 命名空间形态（I-026-003）· 策略接口形态 | 计划（未开工） |
| R2 | 内存供应商 + 双策略（判据 #2/#3）：有界 + TTL 清理 + 驱逐 + 并发安全 | 依赖 R1 | 计划 |
...
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| I-026-001 | required | Cache 端口 API 形态：Go 泛型 vs `[]byte` vs 结构化值；零值/未命中语义。 | 方案冻结 + 退出判据 #1 | R1 | 用户裁决（R1 合同冻结前置） | 待裁决 | — | — |
| I-026-002 | required | TTL 清理语义：惰性（读时清理） vs 后台协程清理；边界与容量来源。 | 退出判据 #3/#6 | R1 | 用户裁决（R1 合同冻结前置；停机语义随选） | 待裁决 | — | — |
| I-026-003 | non-blocking | 命名空间 / key 前缀约定：模块 ID 前缀 vs 独立命名空间参数。 | 退出判据 #1/#4 | R1 | lead 建议 + 用户确认 | 待确认 | — | — |
