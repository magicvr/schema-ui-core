...
| 检查点 | 内容 | 状态 |
|--------|------|------|
| C1 | **信息裁决**：I-001（Job 停机语义）、I-002（grace/超时默认与配置键）、I-003（Store 排空 × 迁移窗口）三条 required 由用户裁决；I-004（日志/指标断言）确认口径 | **已关门**（2026-08-27 用户裁决全部采纳建议：中断标记重跑；10s + `http.shutdown_timeout`；fail-closed 启动期校验；I-004 lead 口径——D-001 accepted） |
| C2 | **合同正文**：停机顺序、HTTP drain（`http.Server` Shutdown 语义 / grace / 超时 / 强制退出）、Job 停机行为、双方言连接关闭顺序、迁移窗口关系、退出码语义落盘（D-002） | **已关门**（D-002 合同 v0.1.0 冻结，2026-08-27） |
| C3 | **审视与关门**：合同自审（self）+ 可选独立审；R1 关门、Root 信息台账回写 | **已关门**（A-001 self `pass` 0 required；2026-08-27） |
...
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| I-001 | required | 停机时运行中 Job 语义：等完成 vs 中断标记重跑（或按 Job 类型分流） | 方案冻结 | C1 | 用户裁决 | **verified** | — | 2026-08-27 用户裁决：**中断标记重跑**（D-001 accepted；合同 §4） |
| I-002 | required | grace period / 超时默认值与可配置键（含超时后强制退出语义） | 方案冻结 | C1 | 用户裁决 | **verified** | — | 2026-08-27 用户裁决：默认 `10s` + `http.shutdown_timeout`（`HTTP_SHUTDOWN_TIMEOUT`）；非法值 fail-closed（D-001 accepted；合同 §6） |
| I-003 | required | Store 排空与迁移窗口重叠时的停机语义（fail-closed？排队？） | 方案冻结 / 实施 | C1（口径）/R2（关闭） | 用户裁决 | **verified** | — | 2026-08-27 用户裁决：**fail-closed = 启动期校验**，无运行时迁移窗口（D-001 accepted；合同 §5） |
| I-004 | non-blocking | 停机是否需日志 / 指标断言（消费 VP-015） | 验收 | C2/C3 | lead 提案 | **verified** | — | lead 口径：结构化日志三事件断言；指标不进分母（D-001；合同 §7） |
