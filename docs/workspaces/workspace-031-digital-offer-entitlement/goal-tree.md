...
*自动同步工作区扁平目标树（树 + 状态表）。更新任一目标状态/进度后必须同步本文件。更新：2026-09-05（R3 关门：GOAL-004 done 3/3，A-002 independent pass；创建 GOAL-005 启动 R4）*
...
└── GOAL-005-r4-evidence-closeout (R4 证据矩阵/边界核账/关门审计 · active · 0/2)
（R1～R3 关门；R4 进行中——最后一个纲领阶段）
```
...
| GOAL-005-r4-evidence-closeout | R4 证据矩阵/边界核账/关门审计 | **active** | 0/2 | GOAL-001-digital-offer-entitlement | 2026-09-05 设立。检查点：C1 证据矩阵（VP-031 判据 1～8 → 测试/代码/审计路径）+ 边界核账 → C2 关门审计（self + codex independent）与 Root 关门。 |
