...
*自动同步工作区扁平目标树（树 + 状态表）。更新任一目标状态/进度后必须同步本文件。更新：2026-09-05（R2 关门：GOAL-003 done 4/4，A-008 independent pass；创建 GOAL-004 启动 R3）*
...
└── GOAL-004-r3-entitlement-validation-telegram (R3 权益核验/消耗 + 可选 Telegram 注册 · active · 0/3)
（R1～R2 关门；R3 进行中；R4 未立项，按阶段渐进开设）
```
...
| GOAL-004-r3-entitlement-validation-telegram | R3 权益核验/消耗 + 可选 Telegram 注册 | **active** | 0/3 | GOAL-001-digital-offer-entitlement | 2026-09-05 设立。检查点：C1 核验/消耗落地（Check 聚合 + Consume §5.2）→ C2 Telegram 命令 Register + 限流桶 → C3 审视与关门（self + independent）。 |
