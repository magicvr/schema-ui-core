...
| R4 | 证据与关门 | **已关门**（第 3 次关门 2026-09-05 · GOAL-005 done 2/2：A-012 `conditional` 2 required（F-007/F-008）→ D-004 fixed ×2 + A-013 closed ×2 → A-014 independent `pass` 0 required → F-001 关门前置加固（A-015 fixed）→ 重新关门） |

## 信息就绪与未知项（P-005）

| ID | 级别 | 所需信息 / 问题 | 影响门禁 | 最晚阶段 | 验证 / 收集动作 | 状态 | 延期 / 复核 | 证据 / 结论 |
|----|------|-----------------|----------|----------|-----------------|------|-------------|-------------|
| I-031-001 | required | 首波权益形态：仅时长、仅次数、或二者并存（一 Offer 一种）。 | 判据 1/3；R2 放行 | R1 | R1 合同冻结并记录用户裁决/方案证据 | **verified** | — | 2026-09-05 用户裁决：二者并存，一 Offer 固定一种（GOAL-002 D-001） |
| I-031-002 | required | 购买状态最小子集（是否要 `pending`，还是同步一拍 fulfilled）。 | 判据 2；R2 放行 | R1 | R1 状态机与事务边界设计 | **verified** | — | 2026-09-05 用户裁决：同步一拍 fulfilled，无 pending（GOAL-002 D-001） |
| I-031-003 | required | 是否允许 Admin 人工发放/撤销权益。 | 判据 3；R3 放行 | R1 | R1 权限与审计边界裁决 | **verified** | — | 2026-09-05 用户裁决：只读 + 作废，不开放人工发放（GOAL-002 D-001） |
| I-031-004 | non-blocking | Telegram 命令清单（若 channel.telegram 启用）。 | 判据 5 | R1 | R1 冻结价目/购买/我的权益命令子集 | **verified** | — | 2026-09-05 默认冻结：price / buy / entitlements（GOAL-002 D-001；用户可否决） |
| I-031-005 | non-blocking | 模块 id（建议 `biz.digital-offer`）。 | 装配 | R1 | 核验模块命名与 Manifest 约定 | **verified** | — | 2026-09-05 默认冻结：`biz.digital-offer`（GOAL-002 D-001；用户可否决） |
