...
`progress: 60%` = 3/5 个检查点完成（R1、R2、R3）。progress 只作展示，不放行阶段、不关闭 finding、不推导 `done`。
...
| I-034-004 | required | 已分组 NodeID 的直接 URL、动态路径和激活态展开矩阵 | 判据 5 / R4 验收 | R3 | 建立 NodeID → route e2e 矩阵，覆盖 sidebar 直接 route、已登记内页/动态路径与折叠后直接进入 | open | R3 完成前复核 | D-005；待测试证据 |
