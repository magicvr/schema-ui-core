...
| 检查点 | 判据 | 状态 |
|--------|------|------|
| **A** | `kernel.RecoveryPointPort` + `internal/backup` provider 落码（含 `<recovery-artifact>` 校验与错误分类），SQLite 侧 restore harness 有可执行测试 | pending |
| **B** | PG 侧可执行路径（Docker 固定版本客户端工具）验证；部分升级/批级快照的用户裁决落盘 | pending |
