...
└── GOAL-003-r2-codec-and-descriptor-m1-m2 [active] (0/4) · R2 M1/M2 codec 与 descriptor
```
...
- **下一步**：R2（双方言迁移 + Store 编解码）。边界与信息门禁已由 **Root `D-016`** 冻结；首个阶段子目标 **`GOAL-003-r2-codec-and-descriptor-m1-m2`**（M1/M2）已按用户确认的 slug 立项（`active · 0/4`）。M3（仓储与谓词改造）与 M4（Backup Port + 关门审计）待 M1/M2 完成后再立项——**不预创建**。
