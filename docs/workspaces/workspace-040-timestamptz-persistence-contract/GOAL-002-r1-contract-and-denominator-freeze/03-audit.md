...
| A-044 | 2026-09-20 | independent | 6ab57dd0 负值分档 + 44 表 / d67497d7 PG 毫秒族复现 + D-021 F-I-005 拆分 | conditional | 2 | `03-audit/A-044-r1-independent-fi002-fi005-pg-ms-and-d021.md` |
...
**开放 required = 0。** 关门向 independent 审计 = **A-046**（`conditional`，判定 F-I-002 `fixed`、F-I-005 `accepted-residual`，**无新 required**）；编排响应 = **A-047**（`pass`）。C2/C3 已冻结；`I-040-001`～`003` 已 `verified`；`I-040-004` 属 R3、不阻断 R1。

**F-I-005 为 `accepted-residual`**（用户 2026-09-20 书面裁决，范围 + 复审触发 + 失效条件见 child `D-021`）——**不得读作哈希已验证**。**

**待用户书面确认**后，编排器方改 `GOAL-002 status: done` 并同步 `goal-tree.md`；**R2 生产 schema 仍未放行**。

**Recommended 仍开（不阻断关门）**：F-I-008、F-I-009、F-I-025（陈旧句本轮已修）、**F-I-028**（mechanism「一律」句本轮已实际改写）。
