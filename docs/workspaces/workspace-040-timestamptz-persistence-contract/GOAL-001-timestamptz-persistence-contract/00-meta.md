...
| R3 | 读写/时区回归、备份有界核对、证据与关门 | **active**（边界与门禁已冻结：Root `D-018`；**R3-A/B = `GOAL-006` `done · 3/3`**（2026-09-21 静默关门；independent `A-002` `fail`/3 required → 修复 → `A-004` 定向复审 `pass`/开放 required = 0）；**R3-C/D 待渐进立项**，编号/slug 已经用户预确认 = `GOAL-007-r3-pg-cross-version-restore-matrix` / `GOAL-008-r3-exit-matrix-and-root-closeout`，见 Root `D-019` §2） |
