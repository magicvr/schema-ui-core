...
- [x] 判据 1：PG 物理类型、SQLite 合同平等物理类型、UTC 语义、NULL/零值、编解码与公共面禁止泄漏均已书面冻结（`I-040-001`）。
- [x] 判据 2：R1 纳入分母的时间列双方言迁移完成并通过 checksum；非时间 `INTEGER` 显式排除。
- [x] 判据 3：写入绝对时刻与读回一致；VP-020 展示/输入时区不漂移；双方言回归含至少一条 PG 路径。
- [x] 判据 4：R1 范围内 SQLite 快照 / PG dump 路径可升级后恢复，或用户书面 residual 明确点名。
- [x] 判据 5：未引入 ORM/第三库/Redis/MQ/多实例；未把 Admin 维护提示或业务域混入。
- [x] 判据 6：退出矩阵与必要独立意见落盘，开放 required = 0，并经用户确认关门。

> **判据勾选依据**：六条逐条证据矩阵见 `GOAL-008/attachments/r3d-root-exit-criteria-matrix-v0.1.md`；用户 **2026-09-21** 书面确认关门（`GOAL-008/01-decision/D-001-root-closeout-user-confirmation.md`；Root `D-020`）。判据 1–5 的可复跑证据（含真实 PG/SQLite 恢复与升级路径）与跨目标开放 required = 0 由 independent 关门审计 `GOAL-008/A-002` 与 `A-005` 确认。
...
| R3 | 读写/时区回归、备份有界核对、证据与关门 | **completed**（边界与门禁由 Root `D-018` 冻结；**R3-A/B = `GOAL-006` `done · 3/3`**；**R3-C = `GOAL-007` `done · 3/3`**（9 dump + 54 restore 格逐格落盘，18 supported 形状校验全通过；`I-041-004`/`I-041-009` verified）；**R3-D = `GOAL-008` `done · 3/3`**（退出判据矩阵落盘、判据 1–5 满足；independent 关门审计 `A-002` `conditional`/开放 required = 0 与 `A-005` **pass**/开放 required = 0；用户报告的 `dev.cmd start` 缺陷 `F-I-101` 已 `fixed` 并经独立复现；**用户 2026-09-21 确认关门**）） |

`progress: 3/3` = 3/3 个检查点完成（**R1、R2、R3 均 completed**）。progress 只作展示，不放行阶段、不关闭 finding、不推导 `done`；本次 `done` 的依据是**判据 6 的用户书面确认**（Root `D-020`），不是 progress 值。

> **关门声明（2026-09-21）**：本 Root 已于 **2026-09-21 经用户书面确认关门**（`D-020`；`GOAL-008` 检查点 C）。判据 1–5 的逐条证据见 `GOAL-008/attachments/r3d-root-exit-criteria-matrix-v0.1.md`；independent 关门审计 = `GOAL-008/A-002`（`conditional`/开放 required = 0）与 `A-005`（`pass`/开放 required = 0）；用户报告的 `dev.cmd start` 缺陷 `F-I-101`（required）已 `fixed` 并经独立复现与启动器实测。**VP-040 自身的波次关闭 / Vision Review 属决策层（`/vision`）动作，不在本次 Root 关门范围内**（见 `D-020` §4）。
...
>
> **R3 关门记录**（2026-09-21）：R3-A/B（`GOAL-006`）、R3-C（`GOAL-007`）静默关门；R3-D（`GOAL-008`）经退出判据矩阵 + self/independent 关门审计后，由**用户 2026-09-21 书面确认**关门（Root `D-020`）。`I-041-004`/`I-041-008`/`I-041-009`/`I-040-004` 均 verified；`I-041-011`（产品级支持组合表述）按用户裁决只保留在退出矩阵与 R3-C 附件。
