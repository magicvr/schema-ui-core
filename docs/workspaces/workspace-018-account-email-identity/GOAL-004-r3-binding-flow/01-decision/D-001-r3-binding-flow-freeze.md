...
**§5 尝试上限**：5 次失败作废挑战须重发（实现级防爆破，MFA 先例口径；非合同条款）。作废后对同一 pending 的后续校验返回 `EMAIL_CODE_EXPIRED`（挑战已不存在，重发即恢复）——A-001 F-004 口径澄清。
**§6 同址重绑冷却（v1.1.0 追加，A-001 F-002）**：对**相同**待校验地址再次 Bind 视同重发，套用 60 秒冷却；**不同**地址为换绑，立即派发（旧址槽随覆写释放）。
