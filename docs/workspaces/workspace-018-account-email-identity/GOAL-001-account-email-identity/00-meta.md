...
| R4 | **证据**：从 017 默认渠道取出校验信；唯一性 fail-closed 可核对；无 IAM 恢复 / 邀请 / 密码策略产品进入本波。 | 依赖 R3 | **已完成**（GOAL-005 done · 端到端经真实 mock 渠道适配器 · self pass） |

`progress` = 已完成阶段数 / 4。当前 **4/4**。Root 关门审计进行中（self 已过 · independent 走 grok build）。
