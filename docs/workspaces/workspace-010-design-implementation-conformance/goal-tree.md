...
└── GOAL-047-w35-dev-db-init-and-bootstrap [active] (0/3) · W35 · dev/test 数据库初始化快捷方式与自举修复（用户报告 PG 实例重置后 `dev.cmd start` 失败）
```
...
**W26（2026-08-26 关门，GOAL-038 done 4/4）**：用户点名三项符合性对齐，全部落地——① 用户邮箱身份读面：`ListUsers`/`GetUser` 同查询投影 email/email_status（无 N+1），`userToMap` 输出 email/emailStatus/emailStatusStyle（badge 语义），users.json 邮箱列 + 详情字段；② 邮件面页面化：迁移 0060 `mail_outbox` 加 channel/delivery_status（portable additive ALTER），Switcher 对 resend/smtp 落 sent/failed 记录、mock 单记 delivered；admin.settings 贡献 `mail`（控制台）+ `mail-outbox`（声明式 table 六列 + recordView 含正文）两独立页并注册 sidebar（menu_mail/menu_mail_outbox，**复用 settings.read 零新权限**）；设置页移除 tab-mail；③ users-invites 撤销补 `requestMapping.path.id=$row.id` + row-action-bindings 契约回归登记。S1 D-001 冻结（I-001～I-003 required closed）→ S2 实施 → S3 回归：Go 全量 0 FAIL + vitest 1116/1116 + tsc 0 + build ok，go 判定 = additive 无影响不暂挂 → S4 A-001 self 关门审计 **pass**（required 0）。Root/VP 保持 active。
...
| GOAL-047-w35-dev-db-init-and-bootstrap | W35 · dev/test 数据库初始化快捷方式与自举修复：`e2e-pgset` 在 `DB_NAME` 不存在时可自举 + 一次性建齐 dev/test 库的快捷方式 + README/QUICKSTART 初始化章节 + 启动错误可操作化（用户报告 PG 实例重置后 `dev.cmd start` 失败） | GOAL-001-design-implementation-conformance | active | 0/3 | 2026-09-21 |
...
- **W32（2026-09-19 立项并当日关门 · GOAL-044 `done · 4/4`）**：R4 三项残余修复——承接 `[workspace-038-batch-operations-and-job-center]` `GOAL-005` 的 `A-001` F-002/F-003/F-004（cross 审计两腿均 `pass`、开放 required = 0 后留下的 low 级项）。用户 2026-09-19 指令：三条都修，先在 workspace-010 开承载子目标、由 038 侧**有界接受**后移交执行。三项均以**跨页面通用能力**落地：① 列级 `valueLabels`（值 → i18n 键，fail-open 回落原始值）；② `refreshTable(tableId)` 定向刷新 seam（保留选择；`reloadList()` 的 ADR-0022 D2 语义以对照测试钉住未变）；③ `activeStatuses` + 行注册表实现空闲不轮询。**变异验证**三处；回归 Go 全量 + vitest + typecheck 全绿；审计 `A-001` self `pass` + `A-002` 三条 recommended 全 `fixed`；038 `A-003` 三条已回填 `fixed`。
- **W33（2026-09-19 立项并当日关门 · GOAL-045 `done · 4/4`）**：用户 VP-038 关门后的两问——① roles 列表页（同样具备导出功能）缺「导出所选」；② 该按钮应位于列表控件行并靠左，而非筛选栏上方。用户 P-004 选型：位置方案 **A**（表格 page-actions 行左侧插槽）、载体 **本区 W33**（VP-038 保持 `closed`）。交付：渲染器**本地扩展** `props.slot = "list-page-actions"` + `props.targetTable`（表格行拆左右两段、左段为插槽宿主、经 CRUD seam 发布；未声明者逐字原路径；目标表缺失/未知 slot 值 **fail-open 原地渲染**）；`users` 页接入插槽；`roles` 页增 `selection.mode=multiple` + `roles-batch-export` 节点（**后端零改动**，R3 冻结分母本就含 roles）。**实施中发现并修复既有隐患**：roles 页新增 custom 节点后两页 table 同索引 → React 复用同一 `SchemaTable` 实例，把上一张表的 `visibleColumns` 渗入另一张（浏览器 e2e 捕获：users 往返后只剩交集列）→ 修复 = **在 SchemaTable 内按表格身份重置表级状态**（曾试「按节点 id 作 key」，因属无键子数组中的混键改动、收益不明确而撤回）+ 回归锁。**变异验证**：`roles.json` 的 `slot` 拼写错 → 2 例红。**用户报告运行期控制台循环**（`Maximum update depth exceeded` 刷屏）→ 浏览器探针 + 插桩计数 + **W33 前基线对照**确认**先于本波次存在**：保存视图 page-actions 宿主 claim 效应把 `availabilityVersion` 当依赖、其 cleanup 又 `release()` 递增该值 → 自激循环；修复 = 拆成「claim（只依赖宿主元素 + tableId + 重试计数）」+「重试（宿主空闲时 +1）」，并新增**永久守卫** `apps/web/e2e/console-health.spec.ts`（真实外壳逐页走查，任何 console.error/pageerror 即失败；**变异验证**：把旧依赖加回即让守卫失败）。**既有 flake 归因**：`s5-denominator-render` 2 例在干净树同样失败（负载敏感），补显式超时后全套件默认超时全绿。回归：vitest **121 files / 1476 tests** + typecheck/build 0 + e2e mvp **17 passed/5 skipped/0 failed**、admin **18 passed/4 skipped/0 failed** + Go 相关包与 docscheck 全绿；审计 `A-001` self `pass`（0 required + 4 recommended）+ `A-002` 响应（3 fixed + 1 accepted-residual）。登记：roadmap §一 roles 触发面 `fixed`，并把左侧插槽加入「本地扩展清单」。Root/VP-010 保持 active 程序容器。

**W34（2026-09-19 立项并当日关门 · GOAL-046 `done · 4/4`）**：用户在 VP-038 关门后报告「无论是用户列表页还是角色列表页，选中列表项并点击『导出所选』时都会报错宣示『未找到』，而并不会正常导出」。诊断为**两个独立缺陷**（均属已交付面的可用性缺口）：① **operator config 漂移**——`configs/config.yaml` 声明 `profile: custom` 且注释自称「= the full admin preset PLUS channel.telegram」，但 VP-038 把 `admin.jobs` 加进 admin preset 时漏改该内联列表 → 模块未装配 → `POST /api/jobs/batch-export` 未挂载 → 路由回落 `error.notFound`（zh「未找到」），而节点属 `admin.users`/`admin.roles`、在所有 profile 照常渲染；② **mvp/demo 同类形态**——两 preset 既无 `jobs.write` 也无 `data.export`，入口同样只有 404/403 一条路。修复：operator config 补 `admin.jobs`；组件按路由**真实的两道门禁**渲染「**可见 + disabled + 说明**」（依据 = 入口不撒谎 + 空插槽宿主的高度契约，见 `E-001` §5 澄清）；提交命中 404 改报「当前部署未启用批量导出」。守卫：`TestOperatorConfigCoversAdminPreset`（operator config 必须是 admin preset 的**超集**）+ 对称判别性可用性用例 + `batch-export-availability.spec.ts`（**双 profile** 契约）。**变异验证 6 处**（含双向：判据只查单门禁各让对应用例红；e2e 去判据 → mvp 分支红）。**真实端到端**：补齐后 users/roles 均 `202 → succeeded` 且结果可下载。回归：vitest **121 files / 1482 tests** + typecheck/build 0 + e2e mvp **18/5/0**、admin **19/4/0**（含 always-on 可用性 spec）+ `go test ./...` 全绿。审计：self `A-001` `pass` → **independent `A-002`（grok build · grok-4.6 · high）`conditional`**（2 required：XOR 单测不具判别性 / e2e 未入仓且 harness `custom` 列表不一致）→ `A-003` 响应两条 `fixed`（并 append-only 更正 self 三处高估），**开放 required = 0**。登记：roadmap「未决项统一登记」新增该缺陷行（`fixed`）。Root/VP-010 保持 active 程序容器；不重开 VP-038/workspace-038。
