...
└── GOAL-038-w26-email-display-and-mail-pages [active] · W26 · 邮箱身份展示与邮件面页面化对齐（用户邮箱绑定显示 / 邮件控制台与出站记录独立页 / 邀请撤销修复）（3/4）
```
...
**W26（2026-08-26 立项，GOAL-038 active 1/4）**：用户点名三项对齐——① 用户邮箱身份绑定在列表/详情无读面（`userToMap` 与 users.json 均缺 email，VP-018 能力遗漏展示）；② 邮件控制台与出站记录移出设置页为独立页面并注册左侧导航，出站记录覆盖全部渠道（含 mock；列表 = 唯一ID/收件箱/主题/发送渠道/投递状态/创建时间，详情含正文），权限沿用 `settings.read` 不新设；③ 邀请管理「撤销」缺 `requestMapping.path.id` 绑定报 MISSING_PATH_BINDING。根因锚定 E-001；**S1 已冻结（D-001）：I-001～I-003 全部 closed——0060 加列迁移 / admin.settings 两页 + menu_mail·menu_mail_outbox 权限复用 / ListUsers 同查询投影；S2 实施中**。
...
| GOAL-038-w26-email-display-and-mail-pages | W26 · 邮箱身份展示与邮件面页面化对齐（用户邮箱绑定显示 / 邮件控制台与出站记录独立页 / 邀请撤销修复） | GOAL-001-design-implementation-conformance | active | 3/4 | 2026-08-26 |
...
- **W26（2026-08-26 立项）**：GOAL-038 active 0/4，挂 Root 直下；三问题根因锚定见该目标 E-001。
