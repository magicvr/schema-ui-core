...
└── GOAL-044-w32-r4-residual-seams [active] (0/4) · W32 · R4 三项残余修复（列值本地化 · 表格定向刷新 seam · 空闲不轮询）——承接 [workspace-038] GOAL-005 A-001 F-002/F-003/F-004
```
...
| GOAL-044-w32-r4-residual-seams | W32 · R4 三项残余修复（通用列值本地化 / 表格定向刷新 seam / 空闲不轮询；承接 `[workspace-038]` GOAL-005 A-001 F-002/F-003/F-004） | GOAL-001-design-implementation-conformance | active | 0/4 | 2026-09-19 |
...
- **W32（2026-09-19 立项 · GOAL-044 `active · 0/4`）**：R4 三项残余修复——承接 `[workspace-038-batch-operations-and-job-center]` `GOAL-005` 的 `A-001` F-002/F-003/F-004（cross 审计两腿均 `pass`、开放 required = 0 后留下的 low 级项）。用户 2026-09-19 指令：三条都修，先在 workspace-010 开承载子目标、由 038 侧**有界接受**（`accepted-residual`，复核触发 = 本目标交付）后移交执行。三项均为**跨页面通用能力**（渲染器表格层），不属 VP-038 交付范围：① 列值逐值本地化（本地扩展，不改 pinned 工件）② 表格定向刷新 seam（刷新指定表格且不清空选择，与 ADR-0022 D2 的边界需裁定）③ 空闲不轮询（仅当目标表格存在非终态行时打点）。Root 保持 active 程序容器；不重开 VP-038/workspace-038。
