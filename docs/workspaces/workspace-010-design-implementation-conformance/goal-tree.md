...
- **W33（2026-09-19 立项并当日关门 · GOAL-045 `done · 4/4`）**：用户 VP-038 关门后的两问——① roles 列表页（同样具备导出功能）缺「导出所选」；② 该按钮应位于列表控件行并靠左，而非筛选栏上方。用户 P-004 选型：位置方案 **A**（表格 page-actions 行左侧插槽）、载体 **本区 W33**（VP-038 保持 `closed`）。交付：渲染器**本地扩展** `props.slot = "list-page-actions"` + `props.targetTable`（表格行拆左右两段、左段为插槽宿主、经 CRUD seam 发布；未声明者逐字原路径；目标表缺失/未知 slot 值 **fail-open 原地渲染**）；`users` 页接入插槽；`roles` 页增 `selection.mode=multiple` + `roles-batch-export` 节点（**后端零改动**，R3 冻结分母本就含 roles）。**实施中发现并修复既有隐患**：roles 页新增 custom 节点后两页 table 同索引 → React 复用同一 `SchemaTable` 实例，把上一张表的 `visibleColumns` 渗入另一张（浏览器 e2e 捕获：users 往返后只剩交集列）→ 修复 = 渲染器按 table 节点 id 作 key + 回归锁。**变异验证**：`roles.json` 的 `slot` 拼写错 → 2 例红。**既有 flake 归因**：`s5-denominator-render` 2 例在干净树同样失败（负载敏感），补显式超时后全套件默认超时全绿。回归：vitest **121 files / 1476 tests** + typecheck/build 0 + e2e mvp 16/5/0、admin 17/4/0 + Go 相关包与 docscheck 全绿；审计 `A-001` self `pass`（0 required）+ `A-002` 响应（2 fixed + 1 accepted-residual）。登记：roadmap §一 roles 触发面 `fixed`，并把左侧插槽加入「本地扩展清单」。Root/VP-010 保持 active 程序容器。
：R4 三项残余修复——承接 `[workspace-038-batch-operations-and-job-center]` `GOAL-005` 的 `A-001` F-002/F-003/F-004（cross 审计两腿均 `pass`、开放 required = 0 后留下的 low 级项）。用户 2026-09-19 指令：三条都修，先在 workspace-010 开承载子目标、由 038 侧**有界接受**（`accepted-residual`，复核触发 = 本目标交付）后移交执行。三项均以**跨页面通用能力**形态落地（`D-001`）：① 列级 `valueLabels`（值 → i18n 键，fail-open 回落原始值；与 pinned `tagMap` 划界）+ jobs 六态接入；② `refreshTable(tableId)` 定向刷新 seam（用当前查询重取该表格且**保留选择**；`reloadList()` 的 ADR-0022 D2 语义以对照测试钉住未变；只读轮询不删 in-flight 键的理由已记录）；③ `activeStatuses` + 行注册表（`publishTableRows`/`tableRows`，ref 支撑）实现**空闲不轮询**，行不可得时保守刷新。**变异验证**：去 `valueLabels` / 让 `refreshTable` 清空选择 / 禁用空闲判定——三处均实测变红后还原。回归：Go 全量 0 FAIL + vitest **119 files / 1468 tests** + typecheck 0；无产品行为回退。审计 `A-001` self `pass`（0 required）+ `A-002` 响应三条 recommended 全 `fixed`；038 `A-003` 三条 `accepted-residual` 已按 P-003 **回填 `fixed`**，并在 roadmap「未决项统一登记」登记「本地扩展登记」项。Root/VP-010 保持 active 程序容器；不重开 VP-038/workspace-038。
