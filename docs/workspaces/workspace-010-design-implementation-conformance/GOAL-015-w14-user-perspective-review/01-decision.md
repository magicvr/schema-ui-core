...
|----|------|-----------------|----------|--------------|-----------------|------|-------------|
| I-001 | required（本波关门）→ **已裁决** | F-01～F-14 的 in-scope / defer / 优先级 | 本波 S4 关门 | 本波 S4 关门 | 用户裁决（P-004） | **closed** | 用户 2026-08-17 书面裁决（D-003）：F-01～F-14 **全部 in-scope**，分批 A→C→D→B 另起整改波次；F-01 新增端点 / F-04 存 messageKey / F-08 直接移除。门禁关闭，S4 可关门 |
...
- **本波（已合法关门）**：S1/S2 只读完成（A-001 self pass）；S3 独立审计（A-002 independent pass，无 required）。S4 曾违规关门（E-002/A-003）后经用户裁决回退（E-003/A-004），随后**以 I-001 用户书面裁决（D-003）为据合法关门（E-004/A-005）**：F-01～F-14 全部 in-scope（分批 A→C→D→B 另起整改波次）+ 三方案选择（F-01 新增端点 / F-04 存 messageKey / F-08 直接移除）。GOAL-015 **done · 4/4**。
- **整改实施（按裁决，另起波次）**：批次 A（F-01～F-04）→ 批次 C（F-08～F-10）→ 批次 D（F-11～F-14）→ 批次 B（F-05～F-07），作为 workspace-010 后续整改子目标。