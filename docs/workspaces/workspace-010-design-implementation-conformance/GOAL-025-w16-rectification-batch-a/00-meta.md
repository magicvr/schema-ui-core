...
- [ ] **S1 · 方案冻结**：F01/F07/F08 设计（端点/schema/存储契约/前端交互）+ 信息项登记。
...
|----|------|-----------------|----------|--------------|-----------------|------|-------------|
| I-001 | required | F01 强制改密门禁的端点白名单（哪些端点允许 must_change_password=1 用户访问） | S1 方案冻结 | S1 | 梳理登录/改密/资料端点与中间件 | open | 待 S1 输出白名单 |
| I-002 | non-blocking | F07 revoke-others 是否 bump token_version 导致当前 access token 也失效 | S1 方案冻结 | S1 | 会话/令牌模型核对 | open | 待 S1 输出结论 |
