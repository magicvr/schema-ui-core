...
| 7 | **用户报告：运行中控制台持续报 `Maximum update depth exceeded`** | 见 §4「控制台循环修复」：**不是** W33 引入（在 W33 之前的 `c6346f13` 上同样复现）；定位为保存视图 page-actions 宿主 claim 效应的自持循环，已修复并加**永久守卫**（`e2e/console-health.spec.ts`） |
...
### 5. 控制台循环修复（用户报告，2026-09-19）

**症状**：应用运行中控制台持续报 `Maximum update depth exceeded. This can happen when a component calls setState inside useEffect…`。

**定位过程（可复现证据链）**：

| 步骤 | 结论 |
|------|------|
| 1. 浏览器探针（临时 spec）逐页统计 console.error：sign-in 后 0 → **users 后 7** → roles 后 13 | 循环发生在列表页，非全站 |
| 2. 插桩计数（临时 console.log）：`table-render` **1284 次** / CRUD `provider-render` 仅 24 次 / `fetch-effect` 5 / `narrow-effect` 9 / `identity-reset` 0 | 循环在渲染树**局部**，且**不是**取数、列收窄或新加的表格身份重置 |
| 3. 继续插桩 `claim-effect`：**608 次**，且伴 `publish el → null → el` 抖动 | 命中 **保存视图 page-actions 宿主 claim 效应** |
| 4. **干净基线对照**：把 `render.tsx`/`schema-table.tsx`/两份 schema 回到 **W33 之前**的 `c6346f13` 后重跑探针 → **同样报错**（users 后 7、roles 后 14） | 该循环**先于 W33 存在**，非本波次引入（也解释了为何所有测试长期全绿：没有任何套件断言 console） |

**根因**：`SchemaTable` 的宿主 claim 效应把 `pageListActionsHost?.availabilityVersion` 列为依赖，而它的 cleanup 会 `release()`，`release` 又会 `availabilityVersion++` 用于通知其它表格 —— 于是「claim → cleanup/release → 版本变化 → 依赖变化 → 重新 claim」自持循环，React 报 max update depth。

**修复**（`schema-table.tsx`）：把两件事拆开——
- **claim 效应**只依赖**宿主 DOM 元素**与 `tableId`（外加一个重试计数），宿主元素稳定 ⇒ 不再随 provider 状态抖动重跑；cleanup 仍负责 `release`（卸载/宿主变化时执行一次）；
- **重试效应**在「宿主存在且当前无占用者」时 `setClaimRetry(n+1)`，即原来 `availabilityVersion` 想解决的问题（同页第二张表等第一张释放后接管），且不会自激。

**守卫（防复发）**：新增永久浏览器用例 `apps/web/e2e/console-health.spec.ts`——走真实外壳（users → roles → users，admin profile 加 jobs），任何 `console.error` / `pageerror` 即失败（仅白名单静态资源噪声）。**变异验证**：把 `availabilityVersion` 加回 claim 效应依赖 → 该守卫立即报出成片的 `Maximum update depth exceeded`（随后已还原）。

**教训**：既有测试矩阵（vitest 渲染片段 + e2e 断言 UI 行为）对「控制台错误」这一类回归**完全盲**；本次起由 `console-health` 兜底。
