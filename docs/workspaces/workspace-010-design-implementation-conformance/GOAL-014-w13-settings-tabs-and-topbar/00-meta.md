...
title: W13 · 设置页 Tabs 化与顶栏/搜索交互打磨 + 个人中心头像上传 + 通知中心交互修正 + 列表筛选即时生效（设置页功能单元 Tabs / 移动端品牌条 / 搜索框组贴合 / 明暗-语种按键对调 / 移动端汉堡靠左 / 头像上传 / 通知点击即读 / 筛选即时生效）
status: done
...
- **范围（本波实施）**：T-01 设置页功能单元 Tabs；T-02 移动端品牌条（含 2026-08-16 追加：汉堡按键靠工具栏最左）；T-03 搜索框组【文本框+搜索键】贴合恒同行；T-04 顶栏亮暗/语种按键对调；**T-05（2026-08-16 追加）个人中心【资料】选项卡头像上传（极其附属功能）**；**T-06（2026-08-16 追加）通知中心交互修正（铃铛条目可点击跳转展开并标已读 / 列表点击展开详情并标已读 / 移除行内标为已读动作 / 未读数即时刷新）**；**T-07（2026-08-16 追加）列表筛选即时生效（下拉等筛选项变动立即重新筛选；文本框+搜索键保持提交式；筛选记录与实际生效条件一致）**。
- **非范围**：不改协议 schema / 页面文档结构约束；不改 Go API；不改 Profile 默认集 / 模块矩阵 / Manifest 装配语义（go 判定：无影响、不暂挂）；不改设置项字段与动作契约；不改个人中心页既有 Tabs 行为。

## 成功标准与路线图（P-001）

- [x] **S1 · 设计冻结**：D-001～D-003 + **D-004（2026-08-16：T-07 列表筛选即时生效设计）**；信息项登记
- [x] **S2 · 实施**：T-01～T-06（E-002/E-005/E-008/E-010）+ **T-07 筛选即时生效（E-013）**
- [x] **S3 · 测试与回归**：相关单测/e2e 更新 + vitest 全量 + tsc + Go 全量（E-003/E-006/E-008/E-011/E-014）
- [x] **S4 · 自审与关门**：A-001～A-004 + **A-005** self 审计 + 台账同步（E-004/E-007/E-009/E-012/E-015）

progress: 由四个等权检查点派生（S1～S4）；当前 **4/4**（2026-08-16 五次关门；A-005 self pass；T-07 回归全绿）。
