...
| W35 | GOAL-047-w35-dev-db-init-and-bootstrap | **active**（0/3 · 2026-09-21 立项） | 用户报告 PG 实例重置后 `.\dev.cmd start` 因目标库不存在失败：提供 dev/test 一次性初始化快捷方式 + 修复 `e2e-pgset` 在 `DB_NAME` 不存在时无法自举 + README/QUICKSTART 初始化章节 + 启动错误可操作化；结构由用户裁决挂本 Root（不新开 VP），slug 经用户确认 |
