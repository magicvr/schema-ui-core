...
| W19 | GOAL-030-w19-my-wallet-lazy-open-empty-state | **done**（4/4 · 2026-08-18） | 我的钱包惰性开通（POST）+ WALLET_NOT_FOUND 空态 |
| W20 | GOAL-031-w20-notification-settings-in-account | **done**（4/4 · 2026-08-18） | 通知设置从列表页迁到个人中心 |
...
| W35 | GOAL-047-w35-dev-db-init-and-bootstrap | **active**（2/3 · 2026-09-21） | 用户报告 PG 实例重置后 `.\dev.cmd start` 因目标库不存在失败：已完成 dev/test 一次性初始化快捷方式 + `e2e-pgset` 自举修复 + README/QUICKSTART 初始化章节 + 启动错误可操作化；A/B 完成，待 self + grok independent 审计；结构由用户裁决挂本 Root（不新开 VP），slug 经用户确认 |
