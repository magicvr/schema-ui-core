...
- [ ] **S1 · 方案冻结**：YAML 字段清单（迁移哪些配置）、优先级与插值规则（未定义 fail-closed）、.env 衔接、env 兼容路径；方案级 self 审视
...
|----|------|-----------------|----------|--------------|-----------------|------|
| I-001 | required | YAML 覆盖哪些配置项（现有 env 全集迁移？保留哪些 env 直读） | S1 方案 | 现 config 清单对照 | open |
| I-002 | required | 敏感项清单与插值规则（未定义时 fail-closed 语义） | S1 方案 | compose :? 先例对照 | open |
| I-003 | required | 现有部署只设 env 的迁移路径（YAML 缺失时行为） | S1 方案 | 向后兼容对照 | open |
