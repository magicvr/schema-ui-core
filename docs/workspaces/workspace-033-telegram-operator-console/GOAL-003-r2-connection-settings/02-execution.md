...
只记录已发生事实；R2 生产代码、测试与 checkpoint 在发生后分别落盘。C2 配置 schema、v67 migration、runtime 回读、settings PATCH 与配置导出接缝已实现并有测试事实；C2 检查点已完成但 GOAL-003 仍为 `active · 2/5`；C3 的 Bot API client、connection manager、会话租约与 UI 等事实须按后续条目追加。
