...
- [x] webhook 模式以 `getMe + setWebhook` 建立连接，polling 模式以 `getMe + deleteWebhook + getUpdates` 建立连接；公网 URL 显式配置且本地 Fake Bot API 可验收。
- [x] webhook 与 polling 可互斥热切换；未绑定时只在控制台 heartbeat 存活期间轮询，已绑定且 polling 时常驻。
- [x] Dispatcher 业务 Register 非空即视为已占用；已绑定会话隐藏人工台。
- [x] 未绑定会话按用户与群分栏落盘，只收录 Telegram 实际投递给 bot 的文本更新；人工可代 bot 发言，无权限时灰掉。
- [x] 保持首波边界：无历史回灌、FSM、群发、频道、多 bot、多实例 polling、SSE/WebSocket；不进入默认 Profile。
- [x] 证据矩阵可核对，相关 required finding 归零后才允许关门。
...
| R3 | 会话与人工台：入站会话落盘、用户/群分栏、未绑定人工 IM、发言权反馈 | **完成**：`GOAL-004-r3-session-operator-console` done · 4/4；C1/C2/C3 已分别由 independent close-out 与 response 关闭；C4 由 A-039 `subagent (gpt-5.6-sol · reasoning medium)` independent `pass`、A-040 response 关闭；Web 构建错误已修复，R3 Root 进入 R4 |
| R4 | 证据与关门：退出判据矩阵、红线核账、审计 finding 闭合 | **完成**：E-029；A-001 self 原始 F-001 已由 E-028/A-002 `fixed`；A-003 `subagent (gpt-5.6-sol · reasoning medium)` independent `pass`；A-004 response；open required = 0 |
...
| I-033-009 | non-blocking | Admin 短轮询间隔 | 判据 5 | R1 | **verified (decision + implementation + independent pass)** | D-002；GOAL-004 A-039/A-040：10 秒单飞、失焦暂停、恢复立即刷新 |
| I-033-010 | non-blocking | 发言权探测与缓存失效 | 判据 5 | R3 | **verified (decision + implementation + independent pass)** | D-002；GOAL-004 A-039/A-040：混合 `getChatMember` 预检 + 发送 403 最终否决；60 秒 bot/chat 缓存，显式重探 |
| I-033-023 | required | C4 `getChatMember` capability API 的承载方式与 60 秒 bot/chat 缓存所有权 | C4 API/UI/关门 | C4 | **verified (decision + implementation + independent pass)** | D-011；A-039/A-040：独立 capability 路由；channel.telegram capability service owner；60 秒 absolute TTL、single-flight、403 失效、`refresh=1` 显式重探 |
...
`01-decision/`、`02-execution/`、`03-audit/` 平铺 ledger；Root execution 已登记至 E-029；R4 关门意见 A-001～A-004 均已落盘。
