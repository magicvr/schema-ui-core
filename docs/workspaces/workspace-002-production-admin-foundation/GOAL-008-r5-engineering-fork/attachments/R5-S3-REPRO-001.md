...
> **取代注记（2026-08-03 · 响应 A-011 F-005/F-006）**：本记录为首次复现尝试——build 排除在计时外（复用预构建镜像）、部分协议 §4 字段不足，不满足 clean-ref 计时与完整可追溯字段要求；不再作为 S3 达标证据，由 [R5-S3-REPRO-002](R5-S3-REPRO-002.md)（clean-ref，build 计入，13.5s ≤ 900s）取代。本记录保留为历史事实。
