...
`attachments/r4-c1-freeze-package-draft.md`（`status: accepted`）。该包含
Contribution 最小字段与 `ContributionIdentity.Key` 校验、Plan 元数据与运行时
...
- Provider surface：framework-agnostic `Provider` + Plan-owned `Registrar`；
  ...
  决定迁移是否编译或执行。
- R4-I002 以该冻结包为 `verified` 证据；GOAL-006 A-004 `F-IND-006-FR-001`
  以此闭合为 `fixed`。
