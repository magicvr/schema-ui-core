...
- 建区完成（E-001 / D-001）；I-001 基线盘点完成（E-002）；I-002 Token 命名 **accepted**（D-002）；**I-005 视觉方向冻结 closed**（D-004 / E-004）；S1–S5 均未勾选；`progress: 0/5`；Root `active`。
- **A-004 / D-003（2026-08-09）已合并响应三审**：原则维持；recommended 入 S1 清单；F-002 映射方案已锁（`--elevation-*` → `--shadow-*`），**完整 fixed 仍待实施证据**。
- **D-004**：Stitch 定稿可作视觉输入；不构成实现完成；不关闭 F-002。
- 开放 required finding = **1**（**F-002**）。
- 到期 required 信息项（S1 实施前）= **0**；可继续 S1 有界实施/验证，但 F-002 合法闭合前不得勾选 S1。
