...
- **当前 active 交付 VP**：[VP-006](../vision/plans/VP-006-full-protocol-contract-v2-7-0.md)（schema-ui-docs@v2.7.0 整份契约可验证兼容；lead / delivery = `workspace-005-full-protocol-contract-v2-7-0`）。**现行协议覆盖权威 = `I-PROTO-FULL-001`**（Root `attachments/I-PROTO-FULL-001-coverage-v2-7-0.md`，12/12 域 include；历史 `I-PROTO-001 v0.1.3` 仅为 MVP 回归基线，只读）。VP-005 视觉产品化实施冻结至 VP-006 closed。
...
- **workspace-005**：状态 **active**，角色 **delivery**，Root `GOAL-001-full-protocol-contract-v2-7-0`（S0–S4 已完成），`primary_plan` = `VP-006-full-protocol-contract-v2-7-0`；整份契约实现与验证台账（覆盖表 `I-PROTO-FULL-001`）。
