...
it("invites.resend posts the resend and copies the rotated link to the clipboard", async () => {
  const writeText = vi.fn().mockResolvedValue(undefined);
  vi.stubGlobal("navigator", { ...navigator, clipboard: { writeText } });
  const fetcher = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    const url = String(input);
    if (url.includes("/api/users/invites/") && (init?.method ?? "GET") === "POST") {
      return new Response(JSON.stringify({ link: "/invite/accept?token=rotated-9" }), {
        status: 200,
    ...
    return new Response(
      JSON.stringify({
        items: [{ id: "inv-abc", roles: ["viewer"], status: "pending", revokable: true }],
        total: 1,
        page: 1,
        pageSize: 10,
      }),
  ...
  const doc = {
    ...
    actions: {
      resendInvite: { type: "custom", handler: "invites.resend" },
    },
    body: {
      type: "table",
      id: "invites-table",
      props: {
        ...
        actions: [{ key: "resend", label: "Resend", actionRef: "resendInvite", permissionIntent: "edit" }],
      },
    },
  } as unknown as RenderPageDocument;
  const container = await renderDocument(doc, fetcher as typeof fetch);
  ...
  if (button === undefined) throw new Error("Resend row action not rendered");
  await act(async () => {
    button!.click();
    await new Promise((resolve) => setTimeout(resolve, 200));
  });
  const posts = fetcher.mock.calls.filter(([, init]) => (init?.method ?? "GET") === "POST");
  expect(posts.length).toBe(1);
  expect(String(posts[0]?.[0])).toMatch(/\/api\/users\/invites\/inv-abc\/resend$/);
  expect(writeText).toHaveBeenCalledTimes(1);
  expect(String(writeText.mock.calls[0]?.[0] ?? "")).toBe("/invite/accept?token=rotated-9");
  vi.unstubAllGlobals();
});
