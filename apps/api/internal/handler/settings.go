...
func settingsPatch(repository SettingsRepository, operations operationlog.Recorder) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		now := time.Now().UTC()
		updated, err := repository.PatchSiteSettings(body.SiteTitle, body.LogoURL, now)
		if errors.Is(err, settingsrepository.ErrInvalidSiteTitle) {
			writeError(w, http.StatusBadRequest, "INVALID_SITE_TITLE", "siteTitle must not be empty")
			return
		}
		if errors.Is(err, settingsrepository.ErrInvalidLogoURL) {
