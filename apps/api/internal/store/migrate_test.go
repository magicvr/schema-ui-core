...
func TestMigrateFreshDB(t *testing.T) {
	...
	if len(applied) != 69 || applied[67].version != 68 || applied[67].name != "telegram_ingress" || applied[68].version != 69 || applied[68].name != "telegram_outbound" {
		t.Fatalf("applied = %+v, want v69 telegram_outbound tail", applied)
	}
	applied = applied[:66]
	...
	for _, tbl := range []string{
		...
		"operation_log", "operation_log_correlation", "operation_log_archive", "operation_log_session", "site_settings", "system_data_reconcile", "system_data_grants", "jobs", "service_credentials", "subjects", "vouchers", "voucher_batches", "telegram_config", "telegram_sessions", "telegram_inbound_messages", "telegram_outbound_messages",
	} {
	...
	if len(applied2) != 69 || applied2[67].version != 68 || applied2[67].name != "telegram_ingress" || applied2[68].version != 69 || applied2[68].name != "telegram_outbound" {
		t.Fatalf("migrations after reopen = %+v, want v69 telegram_outbound tail", applied2)
	}
