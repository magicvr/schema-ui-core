...
func TestRestartPersistence(t *testing.T) {
	...
	if len(applied) != 49 || applied[48].version != 49 || applied[48].name != "seed_admin_must_change_password" {
		t.Fatalf("applied after restart = %+v, want 49 ending in seed_admin_must_change_password", applied)
	}
	applied = applied[:42]
