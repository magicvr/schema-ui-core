...
func TestRestartPersistence(t *testing.T) {
	...
	if len(applied) != 54 || applied[52].version != 53 || applied[52].name != "operation_log_mail_events" || applied[53].version != 54 || applied[53].name != "account_email_identity" {
		t.Fatalf("applied after restart = %+v, want 54 ending in account_email_identity", applied)
	}
	applied = applied[:42]
