...
func TestRestartPersistence(t *testing.T) {
	...
	if len(applied) != 47 || applied[46].version != 47 || applied[46].name != "operation_log_archive" {
		t.Fatalf("applied after restart = %+v, want 47 ending in operation_log_archive", applied)
	}
	applied = applied[:42]
