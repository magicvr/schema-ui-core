...
func TestRestartPersistence(t *testing.T) {
	...
	if len(applied) != 55 || applied[52].version != 53 || applied[52].name != "operation_log_mail_events" || applied[53].version != 54 || applied[53].name != "account_email_identity" || applied[54].version != 55 || applied[54].name != "email_verification_challenges" {
		t.Fatalf("applied after restart = %+v, want 55 ending in email_verification_challenges", applied)
	}
	applied = applied[:42]
