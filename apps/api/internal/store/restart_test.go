...
func TestRestartPersistence(t *testing.T) {
	...
	if len(applied) != 43 || applied[42].version != 43 || applied[42].name != "operation_log_wallet_jobs" {
		t.Fatalf("applied after restart = %+v, want 43 ending in operation_log_wallet_jobs", applied)
	}
	applied = applied[:42]
