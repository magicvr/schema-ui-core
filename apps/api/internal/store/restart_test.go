...
func TestRestartPersistence(t *testing.T) {
	...
	if len(applied) != 45 || applied[44].version != 45 || applied[44].name != "operation_log_service_credentials" {
		t.Fatalf("applied after restart = %+v, want 45 ending in operation_log_service_credentials", applied)
	}
	applied = applied[:42]
