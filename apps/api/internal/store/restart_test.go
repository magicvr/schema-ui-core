...
func TestRestartPersistence(t *testing.T) {
	...
	if len(applied) != 53 || applied[52].version != 53 || applied[52].name != "operation_log_mail_events" {
		t.Fatalf("applied after restart = %+v, want 53 ending in operation_log_mail_events", applied)
	}
	applied = applied[:42]
